...
want, not a name. `npm run find -- "fixed width column beside a growing
pane"` surfaces `FillPaneRailGrid` / `MajorPaneBox` / `NoShrinkScrollBox`;
`npm run find -- "two stacked charts equal height"` surfaces `HalfFillColumn`.
Run this BEFORE step (a) below — it is how you find the name to cite.
