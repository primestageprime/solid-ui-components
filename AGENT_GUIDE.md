...
**Step zero: `npm run find -- "<what you need>"`.** COMPONENTS.md is ~2,000
lines of prose, searchable only by a name you already have to know — which is
exactly how a track-list `Grid` Override went unfound for a task that needed
one. `npm run find` ranks `catalog.json` (generated by `npm run catalog`, one
record per exported component and curried variant — depth, kind, family,
`variantOf`/baked overrides, and every COMPONENTS.md `summary`/`useFor`/`Not
X` clause the entry carries) against a plain-text description of the shape you
want, not a name. A curried variant or factory with no COMPONENTS.md bullet
and no JSDoc of its own is never left blank: its `summary` is synthesized from
its baked overrides plus its base's bullet (`record.summarySource` says which
of `components-md`/`jsdoc`/`synthesized`/`alias` produced it), so `summary:
null` — and therefore invisibility to this search — is now down to 62 BASE
primitives/composites genuinely missing both (`catalogSummaryGaps` in
`scripts/health-baseline.json`; `npm run find` still falls back to a record's
JSDoc `notes` for those). `npm run find -- "fixed width column beside a
growing pane"` surfaces `FillPaneRailGrid` / `MajorPaneBox` /
`NoShrinkScrollBox`; `npm run find -- "two stacked charts equal height"`
surfaces `HalfFillColumn`. Run this BEFORE step (a) below — it is how you find
the name to cite.
