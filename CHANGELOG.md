...
- **`DirtyComboBox` parity with thorcasting's scenario chip** (each opt-in; unconfigured, the control is unchanged): `onCreate` turns the reset into an always-standing split `[ ↺ │ + ]` (reset disabled while pristine) whose "+" creates, selects and opens the new item's name field; `onRename` makes the name click-to-rename (Enter or blur saves, Esc cancels) with the caret as the menu button; `labels.none` leads the menu with "None" (`DIRTY_COMBO_NONE_ID`); `DirtyComboItem` gains `color`/`shape` (a swatch on the row and the selected value) and `disabled`/`reason`. Model: `dirtyComboCreate`, `dirtyComboRename`, `dirtyComboUniqueLabel`; the view gains `none` and `canRename`. **`ScenarioComboBox` now names all three**, so its menu leads with "None".
- **`EditableTitle`: `autoEdit`** — opens the editor at mount, focused with the text selected.
- **`SlideReveal`** — slides its children in and out horizontally (its box grows `0fr ↔ 1fr` + fade, 180ms; instant under `prefers-reduced-motion`), so a frame around it grows with it; collapsed children stay mounted but `inert`.
- **`ReservedWidth`** — the invisible outer element that holds a component's widest state (an invisible, inert copy in the same grid cell), so an animation inside it never moves its neighbours while the visible content keeps hugging itself.
...
- **`LevelsTimeline`: numbered, hoverable, draggable dated events.** Flags are numbered 1…n in time order in one two-digit box (`Mutation.label` is now announced, not painted); overlapping boxes nudge apart with a leader to their rule. Hover/focus shows the exact day plus the new optional `Mutation.details`. New optional `onMoveMutation(id, at)` makes flags draggable (day-snapped, clamped between neighbours; arrow keys move a day). The bottom axis is dated: a tick at every flag day, cadence ticks by span (weekly ≤ ~4 months, monthly ≤ 15, quarterly ≤ 35, else yearly), and non-overlapping horizontal labels via the new exported **`abbreviateDates`** (also for change-tab chips). Non-selected rules are more visible. Fixed: flag boxes clamped to 640 rather than the measured width on wide cards.
- **`createAxisWaterMarks`** hook — a held y-domain that expands with the data at once and shrinks only on `reset()` (ceiling and floor, both on `createHighWaterMark`), with the pure `holdFitDomain`, `createHeldFit`, and the headless `observeAxisWaterMarks` table. Promoted from thorcasting. **`LevelsTimeline`: `levelsValueFit(levels)`** gives it the rails' value extent; the showcase composes it with `valueDomain` and a shrink `IconOnlyButton`.
- **`LevelsTimeline`: pick strategies.** New curried `pickAt?: PickStrategy` and exported **`pickDay`** (whole UTC day under the pointer, clamped to the domain). Without a strategy the chart keeps its nearest-month pick, now deprecated as a default. The hover readout follows the strategy and refreshes when data changes under a still pointer.
- **`Icon`: `lock`, `arrows-up-down`, `fit`, `link`, `fullscreen`, `fullscreen-exit`** — the chart y-axis strategy glyphs (locked / full auto / auto-grow + manual shrink; `fit` is a double-headed diagonal inside an axis corner, distinct from `expand`/`shrink`) and a chain for linked sliders; `fullscreen` / `fullscreen-exit` are four rounded corner brackets pointing out / in (no arrows — `fit` is the one inward-arrow glyph). Grouped under `ui`, `navigation` and `actions`.
- **`ChartFrame`** (factory `createChartFrame({ height })`, plus `CHART_Y_AXIS_MODES` and `chartYAxisModeInfo`) — every chart's frame: a title, a vertical y-axis title, and fullscreen plus a Y-axis strategy split button (Auto-grow | manual shrink / Full auto / Locked, in ScrubChart's `auto` / `autoscale` / `fixed` vocabulary). Controlled mode, a stated in-flow height, and fullscreen with no remount.
- **`VerticalAxisTitle`** — Text variant: a sublabel written bottom-to-top, for a chart's y-axis title ("Salary ($)").
- **`MutationSliders`: `readout: "beside"`, `precision`, `grouping: "link"`, items, Reset | Delete** — all additive overrides (defaults unchanged). `"beside"` (type `MutationSlidersReadout`) draws the old amount beside the prior arrowhead, the new amount under the dial and a two-line difference (`+$5K` over `(4%)`) right of the bar. `precision` makes the amount editable in place (rounded to that power of ten, clamped, moves the dial like a key press). `grouping: "link"` adds a per-dial link toggle over the same `selected` selection; a linked group moves by the same AMOUNT. An entity's optional `item` groups consecutive dials of one item into an `itemFrame`. `onReset` with `onRemove` gives each dial a split Reset | Delete footer. New curried variant **`CompactCurrencyMutationSliders`** locks compact dollars, `"beside"`, `snap: 1_000`, `"link"`, `precision: -3` and `ItemTintSurface`.
- **`GutteredFillStretchRow`** — Layout variant: `FillStretchRow` at the new `xl` (24px) row gap, for instruments whose labels run into the gap between them.
- **`ItemTintSurface`** — Surface variant: borderless, padding-free, faint accent wash; the frame round one item's dials in `MutationSliders`.
- **`MarkedSlider`: `priorLabel`** — optional text beside the prior arrowhead; a `deltaLabel` containing a newline draws as lines.
- **`HotkeyButton`: `hotkeyFires(press, hotkey)` + `HotkeyPress`** — the pure predicate the button's key listener uses, exported for consumers that bind the same key elsewhere.
- **`RateGauge`: `callouts?: "leaders" | "corners"` and `formatCornerDelta?`** — overrides. `"corners"` drops the leader lines for right-aligned corner blocks (value, standing and difference top-right; the reference bottom-right) so the dial keeps its height. The gauge never picks its own layout: a product curries one gauge of each and its layout picks at one breakpoint. `formatCornerDelta(delta, baseline)` words the corner difference line (default: `formatDelta`'s line plus a whole percent of the baseline, e.g. `+50,000 (67%)`).

### Changed

- **`FullscreenBox`: the default corner button draws `fullscreen` / `fullscreen-exit`** (corner brackets) instead of `expand` / `shrink` (arrows), matching `ChartFrame`; an inward arrow now means only `fit`. No consumer uses the default corner.
...
- **`CurrencyInput` always shows a literal `$`** (`currencyDisplay: "narrowSymbol"`): the locale-driven default rendered USD as `US$` in en-GB / en-CA browsers. International currency comes later.
...
- **`RateGauge`: leader lines never cross or run on top of each other.** Leaders are now ORTHOGONAL past a radial stub — stub, then (only if the row moved) a vertical channel, then the run to the label column — nested so the higher rising row takes the channel further left (mirrored for falling rows), channels ≥ 4px apart, and each stub cut to its own length along its ray so exits bunched at a pole spread apart. Replaces the gutter-plus-diagonal dogleg, whose near-vertical doglegs all sat within a pixel of each other and crossed at the top of the arc (1,154 of 6,885 readings over four box sizes had a crossing or a stacked segment; now none cross, and only needles within ~2.5° of each other leave parallel runs under 2px apart). New pure `leaderConflicts` (private geometry) is the check. Visible change: every leader that moved off its natural height now steps instead of slanting.
- **`RateGauge`: leader callout rows stay inside the gauge's box.** The clamp held only row CENTRES, by a margin sized for one line, and moved the whole block rigidly: a two-line edge row's first line sat ~5px above a height-bound box, and a reading spanning pole to pole was pushed off the bottom to rescue the top (a 320×300 box drew text from −5 to 313px). Each row is now held half its own block clear of both edges and the pitch is restored in both directions, so the leaders bend instead; rows keep their order, so leaders still never cross. Visible change: edge rows sit a few px further in. Every sweep stop from −domain to +domain now fits a box ≥ 92px tall.
