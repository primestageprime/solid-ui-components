...
## SlideReveal
- **SlideReveal** — Atomic (Depth 1). Owns `SlideReveal.css`, no component imports. Slides its children in and out: they are uncovered from the left edge (a `clip-path`) while they fade, in 180ms ease-out; instant under `prefers-reduced-motion`. **The rule (Peter, 2026-09-24): an animation must hold enough space that it never shifts the layout of other components** — so the reveal ALWAYS reserves its children's full width and animates only what is visible inside that box, never its size (a first cut animated a `0fr ↔ 1fr` grid column, which pushed neighbours; don't). Hidden children stay MOUNTED, marked `inert` + `aria-hidden`, so a hidden button can be neither tabbed to nor clicked; the clip insets by -4px so a focus ring is never cut off. Key props: `when` (`boolean`), `children`. Use for: actions that appear only in a state (DirtyComboBox's save segment and reset). Consequence to design for: the space is taken even while hidden.
