...
## Placeholder
- **Placeholder** — Composite (Depth 2, owns CSS). A themed "fill me in" box for section/tile SKELETONS during the sections-first build phase, before real components land — it shows how pieces will ARRANGE without inventing throwaway markup. Owns only appearance + width/min-height/flex (no grid geometry — centering comes from the composed `CenteredStack`, label from `TextSublabel`), so it stays layout-pure. No `class`/`style` prop on any curried variant — every shape is compile-time config; a call site that needs a shape this file doesn't have gets a new named variant here, never a per-instance override. Two behaviour axes plus tile-size presets are baked into curried variants (the raw `fit`/`multiline`/`size` props on the base are escape hatches for these factories only): `FitPlaceholder` (shrinkwraps to its label, single line — chips/tags), `FillPlaceholder` (fills width, single line — bars/inputs), `BlockPlaceholder` (fills width, an open block with **no natural height of its own** — it grows to fill remaining space in a flex column via baked-in `flex:1`, inherent to what "block" means, not an opt-in; paragraphs, content areas, detail panes), and the tile presets `SmallPlaceholder` (60px), `MediumPlaceholder` (120px, KPI tiles), `LargePlaceholder` (200px, chart/table tiles) — fixed-size grid tiles, which deliberately do NOT grow (a tile has a size; a block doesn't). Data prop: `label`. Factory: `createPlaceholder`. Type: `PlaceholderDataProps`, `PlaceholderProps`, `PlaceholderSize`. Use for: skeleton dashboards / section stubs while composing a page before the real content exists.

## PickNumberLabel
- **PickNumberLabel** — Atomic (Depth 1, owns CSS). Renders `N)` — the "press 1" / "press 2" key hint beside each option in a numbered choice (a pairwise comparison, a pick-one-of-N list). Exists so the hint is a component with a single typographic treatment rather than a string literal styled per call site. `pulse` acknowledges a registered pick: pass any value that CHANGES when the pick happens (an incrementing counter, a timestamp) and each change replays a one-shot scale-bounce — `undefined`, and the first defined value, render inert, so a freshly-mounted label never pops on arrival. The replay works by removing and re-adding the animating class with a forced reflow in between (toggling a boolean `true → true` would not restart a CSS animation), all internal — never a call-site `class`. Data props: `number` (required), `pulse?`; standard `<span>` attributes pass through. Type: `PickNumberLabelProps`. Use for: pairwise ranking screens, numbered choice lists, any keyboard-driven pick where the number IS the affordance.

## BusyOverlay
- **BusyOverlay** — Composite (Depth 2, owns CSS). Composes `Icon` + `CenteredStack`. The "this region is working on it" state for a region whose existing content should stay on screen underneath: a spinner over a dimmed scrim (`rgba(0,0,0,0.55)`), anchored `position:absolute; inset:0` to its nearest positioned ancestor — an image frame, a chart canvas, a panel. Dims rather than replaces, so the thing you were looking at is still the thing you're looking at, and swallows pointer events (`cursor: progress`) because the content underneath is mid-change and clicking it would act on state already being replaced. The spinner is `Icon name="spinner"`, the one icon name that animates itself. Announced as `role="status" aria-live="polite" aria-busy="true"` — worth saying, not worth interrupting a screen reader mid-sentence. Sibling of `InlineChartErrorOverlay`, which does the same job for a FAILURE. **Not** a progress bar substitute: use this when there is no honest percentage to report (one opaque round trip), and `AsyncProgress` / `StackedProgressBar` when there is. Data prop: `label?` (short statement — "Cropping", "Saving"; omit when the surrounding UI already says); standard `<div>` attributes pass through. Type: `BusyOverlayProps`. Use for: image transforms, saves, refetches — any single awaited operation over content that stays put.

## FramedImage
- **FramedImage** — Atomic (Depth 1, owns CSS). The FRAME around real photographic content. The image bytes themselves stay a plain `<img>` (no design system should stylize a photo), but the frame — size, overflow, `object-fit`, backdrop — is a design decision like any other and lives in component-owned CSS, never a call-site `style`. Extracted from the pattern `MediaCard` uses internally so any consumer gets it without depending on the whole card. Two fit modes: `"cover"` fills the frame and crops the overflow (a fixed-aspect thumbnail slot), `"contain"` fits the whole image inside and letterboxes (a detail view where nothing may be cropped away). `backdrop` decides what shows through those letterbox bars: `"surface"` (default, `--sui-bg-tertiary` — the frame reads as themed UI) or `"dark"` (a fixed near-black stage, `--sui-photo-backdrop`, deliberately theme-INDEPENDENT — for a photo viewer/lightbox where the photo should be the only thing on screen with colour in it). `rotationDegrees` previews an accumulated, not-yet-applied rotate edit; `overlay` stacks arbitrary content over the image filling the frame (the frame is `position: relative` for exactly this — e.g. `CropRectOverlay`); `crossfade` (opt-in, `crossfadeDurationMs`, respects `prefers-reduced-motion`) dissolves between images when `src` changes while the frame stays mounted, for a slideshow. **Curried-only at call sites** — `fit`/`squareSize`/`backdrop` are compile-time shape: `SmallSquareThumbnail` (56px, cover), `MediumSquareThumbnail` (120px, cover), `ContainedPhoto` (fills container, contain), `ContainedPhotoOnDark` (same, on the dark stage). NO curried size scale beyond those — an image frame answers "square thumbnail or fill its container", not a px ladder. Data props: `src`, `alt`, `rotationDegrees?`, `overlay?`, `crossfade?`, `crossfadeDurationMs?`, `class?`. Types: `FramedImageProps`, `FramedImageDataProps`, `ImageFit`. Use for: media pickers, photo detail panes, lightboxes, crop/rotate editors.
...
## MediaCard
- **MediaCard** — Composite (Depth 2, owns CSS). A `Card`-family sibling of `EntityCard` for the shape neither `EntityCard` nor `ActionListItem` covers: a fixed-size thumbnail on the left, a name/tags/timestamp column on the right. Not a retrofit of `EntityCard` — `EntityCard` is deliberately pure layout with no typed value renderers, which is exactly why it has no real image region (an image needs a fixed-size CSS-owned frame, not a JSX blob-of-anything slot) and no real tags region (a tag list needs its own renderer). MediaCard's `tags` reuses `TagPill`/`TagPillData` directly — the same primitive `ActionListItem` uses — rather than a second tag-pill renderer. Layout: `thumbnail` (left, fixed 56×56, required) beside a column of `identifier` (top, required) · `tags` (pill row, hidden when empty) · `timing` (bottom); unused rows collapse. Same selection affordance (3px left border + accent wash) and hover-revealed `✕` remove control as `EntityCard`, so a list mixing both cards reads as one family. Key props: `thumbnail` (required, `JSX.Element`), `identifier` (required), `filename?` (rendered as a native `title` tooltip on the identifier region — set when `identifier` is a friendly name distinct from the underlying file), `tags?` (`TagPillData[]`), `timing?`, `selected?`, `onClick?`, `onTagClick?` (stops propagation, never toggles selection), `onRemove?` (hover ✕). Override prop: `class`. Use for: photo/media pickers, file browsers — any list/sidebar card keyed by a thumbnail image plus tags.
...
- **GroupedTable** — ⚠️ **DEPRECATED 2026-08-04, scheduled for removal** (dside `sui`#12546): zero call sites in any consumer, confirmed across all seven SUI-dependent repos and a GitHub code search of the whole org. Nothing is wrong with it — it declares its own props rather than extending `BaseTableProps`, which is the pattern the other renderers were fixed to follow — it has simply never been used. Do not reach for it in new code; if you genuinely need rowspan grouping, say so on #12546 and it will be un-deprecated rather than removed. Table with rowspan grouping for merged cells. Key props: `rows` (array of `GroupedRow`), `columns` (array of `RowspanColumn` with `rowspan` flag), `maxHeight`, `compact`. Use for: tables where rows share common group fields (e.g., vessel calls with multiple trains).
- **VirtualTable** — ⚠️ **DEPRECATED 2026-08-04, scheduled for removal** (dside `sui`#12546): zero call sites, same survey. **It is not a drop-in for `BaseTable`** despite `VirtualTableProps<T> = BaseTableProps<T>` asserting exactly that — it renders its own `sui-virtual-table__*` class namespace with padding/font/colour set inline per cell rather than from the shared `Table.css`, so it looks different; and it ignores four of the props it declares (`spanRow`, `rowActions`, `fixedLayout`, `fit`), which land in `others` and are spread onto a `div`. Renders only the visible window using `@tanstack/solid-virtual`. If virtualisation is wanted again, build it as an opt-in capability of `BaseTable` rather than reviving a second table implementation.
