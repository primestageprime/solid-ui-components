...
## Icon
- **Icon** — SVG icon component with 47 named icons across 8 groups (status, navigation, data, time, actions, UI, auth, cache; includes `pause`, `agent`, `dependency`, `edit`, `trash`, `gear`, `zoom-in`, `zoom-out`, `undo`, `reset`). **`gear` and `settings` are different glyphs**: `gear` draws six teeth on a rim around a hub hole, and `settings` draws a hub with eight rays. Reach for `gear` when the affordance opens configuration, because a reader finds a gear by the teeth; `settings` reads as a sun or a brightness mark. **`refresh`, `reset` and `undo` are three different marks**: `refresh` draws two arcs and two arrowheads and reads as a repeating cycle; `reset` draws a near-closed ring with ONE arrowhead and reads as a return to the start; `undo` draws an open hook with the arrowhead at its left end. Reach for `reset` when the control restores a starting value, and for `undo` when it steps back through a history. **`zoom-in` and `zoom-out` extend the `search` family**: they keep the `search` lens and handle, and add a plus or a minus inside the lens. Reach for the pair for any scale control, such as a chart axis that fits to the visible data or to all of it. Key props: `name` (e.g., `check`, `warning`, `chevron-down`, `search`, `spinner`), `variant` (`outline`|`solid`), `size` (`xs`|`sm`|`md`|`lg`|`xl`). `variant`/`size` are Overrides — curry them via `createIcon`; **`InlineMetaIcon`** (outline, xs) is the shipped variant for icon-beside-sublabel meta rows. Use for: all iconography. Spinner icon auto-animates.
