...
- **formatDateTimeRange(start, end?, mode?)** — Pure-function string helper exported alongside `DateTimeRange`. Same formatting rules; suitable for use inside Primitives that need the formatted string but cannot compose the `DateTimeRange` Composite (e.g. `TitledTimeRangeHeader`).
