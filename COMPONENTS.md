...
## DirtyComboBox
- **DirtyComboBox** — Composite (Depth 2), owns no CSS. Composes `CompactDropdown` (trigger slot + `itemAction` + `onItemDelete` + `DropdownFitLabel`), `SlideReveal`, `TagPill`, `TextSublabel`, `Icon`, `SmallGhostButton`/`IconOnlyButton`, `VerticalDivider` and the `ClusterRow`/`TightClusterRow`/`LooseClusterRow` Layout variants. A list of things that can be SAVED or RESET, with dirty detection: `[Baseline] vs [name ▾ │ ✓]  ↺`. PRISTINE (draft equals saved) shows no save and no reset; DIRTY slides the save segment out of the combo and the reset in beside it (180ms). The reset sits a loose gap away so it is hard to fat-finger — deliberately no confirm. Every row but the selected one carries a trash (Delete/Backspace on a focused row does the same); the list scrolls; the name is as wide as the LONGEST label (capped near 30 characters, ellipsised), so the combo never jumps when the selection changes. Key props (data + callbacks only): `reference` (`string`, the static left pill), `items` (`DirtyComboItem[]`, `{ id, label }`), `selectedId`, `view` (`DirtyComboView`, from `dirtyComboModel`), `onSelect(id)`, `onSave()`, `onReset()`, `onDelete(id)` (never called for the selected row). Use for: a scenario/preset/draft picker whose selection carries unsaved edits (thorcasting's payroll scenario strip).
  - Example:
    ```tsx
    import { DirtyComboBox, viewOf, selectItem, saveDraft, resetDraft, removeItem } from "solid-ui-components";
    ...
    />
    ```
- **dirtyComboModel** — Pure helper (not a component), the headless observation behind DirtyComboBox. `dirtyComboModel(items, selectedId, draft, saved, equals = plainEqual) => DirtyComboView` = `{ dirty, canSave, canReset, deletableIds, widthCh, selectedLabel }` (`widthCh` is the longest label in characters, clamped to `[MIN_WIDTH_CH, MAX_WIDTH_CH]` = `[4, 30]` — a headless reading; the pixel width comes from `DropdownFitLabel`). `plainEqual` is structural equality over plain data. Store transitions over a `DirtyComboStore<C>` (`{ items: SavedItem<C>[]; selectedId; draft }`): `viewOf(store)`, `selectItem(store, id)` (loads that item's saved config — an unsaved edit on the one being left is DISCARDED, no prompt), `saveDraft(store)`, `resetDraft(store)`, `removeItem(store, id)` (refuses the selected row). `widthChOf(items)` is exported too.

## SlideReveal
- **SlideReveal** — Atomic (Depth 1). Owns `SlideReveal.css`, no component imports. Slides its children in and out HORIZONTALLY: the box animates between zero width and the children's own intrinsic width (a `0fr ↔ 1fr` grid column, so nothing is measured in JS) while they fade, in 180ms ease-out; instant under `prefers-reduced-motion`. Collapsed children stay MOUNTED, marked `inert` + `aria-hidden`, so a hidden button can be neither tabbed to nor clicked. The inner box clips horizontally only, so a child's focus ring is not cut off. Key props: `when` (`boolean`), `children`. Note: a collapsed reveal is still a flex item, so its parent row's gap remains. Use for: actions that appear only in a state (DirtyComboBox's save segment and reset).
