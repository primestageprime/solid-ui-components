...
## ChartFrame
- **ChartFrame** — Composite (Depth 2), zero CSS. Composes `FullscreenBox` + Layout variants + `TextTitle` / `VerticalAxisTitle` + `ButtonGroup` + `IconOnlyButton` + `Icon` + `RightPopoverMenu`. Every chart's frame (Peter's chart visual language, 2026-09-24): a **title** top left, a **y-axis title** naming the axis and its units written bottom-to-top along the left edge, and a **button set** top right — fullscreen (the `fullscreen` corner-bracket icon, `fullscreen-exit` while full screen; never arrows), then the **Y-axis strategy split button**. The split button's main face shows the current mode's icon and does its job; the ▾ (accessible name "Y-axis mode") picks the mode. Modes use ScrubChart's vocabulary (`ChartYAxisMode` = `ScrubChartYAxisMode`): `auto` "Auto-grow | manual shrink" (`fit` icon — the face shrinks the axis to fit), `autoscale` "Full auto" (`arrows-up-down` — the face is **disabled**, nothing to shrink), `fixed` "Locked" (`lock` — the face opens the caller's lock editor). The frame owns no axis state: `yAxisMode` / `onYAxisModeChange` are controlled, `onYAxisPress` reports the face, and the caller resets its water marks (`createHighWaterMark`) or opens its dialog. **Height is stated** (a curried Override: px, or `"fill"` for a parent of definite height) so a self-sizing chart can't make the frame viewport-tall; in full screen the same element fills the viewport with no remount, so held axis marks survive. Key props: `title`, `yTitle?`, `yAxisMode?` (omit → no strategy button), `onYAxisModeChange?`, `onYAxisPress?`, `actions?` (the chart's own controls — a Cap field, a span picker — drawn in the header's right-hand cluster BEFORE fullscreen and the y-axis split; keep them toolbar-sized, e.g. a `size="sm"` field at 29px, so the header keeps its height; omitted, the header's DOM is unchanged), `fullscreen?` / `onFullscreenChange?` (controlled, or owned), `children` (the chart; give it a fill height, e.g. `chartHeight="fill"`). Factory `createChartFrame({ height })`; curried variants **`ChartFrame`** (`CHART_FRAME_HEIGHT` = 320px) and **`FillChartFrame`** (`height: "fill"` — takes a parent of definite height, e.g. a BuilderBoard chart panel). Also exported: `CHART_Y_AXIS_MODES`, `chartYAxisModeInfo`. Use for: the frame around any chart on a screen.
  - Example:
    ```tsx
    import { ChartFrame } from "solid-ui-components";
    ...
    <ChartFrame
      ...
      onYAxisPress={() => (mode() === "fixed" ? openLockDialog() : marks.reset())}
    >
      <PayLevelsChart … valueDomain={domain()} />
    </ChartFrame>
    ```
- **YAxisLockDialog** — Composite (Depth 2), zero CSS. Composes `PrimaryConfirmationModal` + `CurrencyInput` or `ThemedNumberInput` × 2 + `TightStack`. The lock editor ChartFrame's `fixed` mode opens: Y max over Y min, validated by the pure `checkLock` (both numbers, min < max), each error on the field at fault and shown only after a Confirm. The fields open **already holding the lock** — the draft is derived ("the reader's edit, else `lock`"), never copied in after the inputs mount, and closing drops the edits. Key props: `open`, `lock` (`YAxisDomain | null`), `onLock(lock)`, `onClose`. The words are an Override (`createYAxisLockDialog({ labels: { title, description, confirm, max, min, notANumber, notAboveMin }, field? })`); curried variants **`YAxisLockDialog`** (money: `CurrencyInput` fields) and **`YAxisLockDialogNumber`** (a COUNT axis — hours, headcount: `ThemedNumberInput` fields, no "$"), both saying "Lock the y-axis" / "Lock" / "Y max" / "Y min". The input is the `field` Override (`"currency"` default, or `"number"`; type `YAxisLockField`). Wire it to `createYAxisStrategy`: `open={axis.dialogOpen()} lock={axis.lock()} onLock={axis.setLock} onClose={axis.closeDialog}`.
...
## RateGauge
- **RateGauge** — Composite (Depth 2). A right-facing half ring — a "D" open to the left — that answers one question: is this value above or below its reference, and by how much? The upper half is the positive zone, the lower half the negative one, and **they split at the angle of ZERO, always the horizontal** (derived via the domain, so an asymmetric domain still splits at its real zero, not at a hardcoded midline). Whichever half the needle is not in dims, so the live half reads as lit. Three marks: a dashed grey needle at the reference, a solid needle in the active tone at the value with a cap arc concentric with the ring, and a curly brace outside the ring spanning the angular difference — plus the faint sector between the needles, which is that same difference drawn as an area. Each mark carries a HUD callout: a terminal on the dial, an elbowed leader out to a shared label column, and a two-line label on the end of it; a spacing heuristic keeps each row as near its own mark as legibility allows and cannot let two leaders cross. When the two needles coincide the three rows collapse to one naming both. Composes `RateGaugeCanvas` + `Text` (`EllipsizedHudCaption`) + `Tooltip` — the value's name is consumer-supplied and non-enumerated, so it ellipsizes in a `<foreignObject>` and offers itself whole on hover. **It owns no CSS and renders no intrinsic element**: the instrument is `RateGaugeCanvas` (Structural Primitive, Depth 1, private to this directory), which owns `RateGaugeCanvas.css` — structural SVG geometry, stroke widths, dash patterns, band fills and `foreignObject` clipping, which no atomic variant can express and none of which is layout. The canvas draws the `<foreignObject>` slot and calls back for its content, because a Primitive may not import the sibling `Tooltip`; the Composite supplies that device and keeps the WORDING (`formatAgainst`, `formatDelta`, the announcement, and which lines are unbounded). ADR 0010's core/adapter split at the element level.
  - **Every word about the numbers is the consumer's.** The component ships no units, no currency and no domain nouns. `formatAgainst(value)` returns the WHOLE second line of a callout ("$60k/yr over breakeven", or "12 points clear") and `formatDelta(delta)` the WHOLE brace line ("$20k/yr to payroll") — sentence builders, not number formatters, because the component adds no words around them. `formatDelta` is handed the raw signed delta (value − reference) and what that difference MEANS is the consumer's to say: a payroll reading flips the sign on purpose, and the gauge must not second-guess that by printing a sign of its own. Defaults are generic: a plain grouped number, "at zero" for zero, and a signed number for the delta.
  - Key props: `domain` (`readonly [number, number]`, the consumer's own units, mapped onto [−90°, +90°]), `baseline` (the reference needle), `value` (clamped to the domain — the picture is clamped, so `aria-valuenow` and every announced figure are the DRAWN values, never one the dial contradicts), `label` (the value's name), `caution?` (the caution threshold: positive values below it take the warning tone, at or above take success; ignored when zero or negative), `baselineLabel?` (default `"Reference"`), `formatAgainst?`, `formatDelta?`. **`caution` is an absolute value in the consumer's units, not a fraction** — a percentage-of-reference rule cannot reach every case: put the reference below a small positive value and a threshold worth 5% of that reference is one the value is already far past, so that consumer needs a flat threshold. Renders as `role="meter"` with an `aria-valuetext` that quotes the callouts word for word, plus the band it stands in when a `caution` threshold names one — a screen reader and a sighted reader can quote the gauge to each other.
  - **Responsive with no size prop**: fills its container at one unit per CSS pixel, so the DIAL grows with the card while labels, strokes and dots keep their own size. The viewBox is cut to the drawn content, and the svg keeps the default `xMidYMid meet` — the ANGLE is the reading, so it must never stretch to a box's aspect. Unmeasured it draws its default dial; an unmeasured gauge is a correct gauge, not a blank one.
  - Factory: `createRateGauge({ baselineLabel, formatAgainst, formatDelta })` — the WORDING is presentational and curries once at the consumer's design-system layer. **`callouts?: "leaders" | "corners"`** (override, default `"leaders"`, which renders exactly what it always did): `"corners"` drops the leader lines and sets the words as right-aligned CORNER blocks — the value's name, where it stands and the difference top-right, the reference's name and where it stands bottom-right (`CORNER_BLOCKS`, one constant) — so the dial keeps the height. **The gauge never picks its own layout**: a product curries one gauge of each and its LAYOUT picks between them at one breakpoint, **`calloutModeFor(box, labels)`** (exported from the root — the one geometry function that is; types `RateGaugeBox`, `RateGaugeCalloutMode`) — feed it **`rateGaugeCalloutLabels(props)`** (exported: EXACTLY the texts the leader callouts draw, built from the same props — `domain`, `baseline`, `value`, `label`, `baselineLabel`, `formatAgainst`, `formatDelta`; type `RateGaugeCalloutLabelProps`; RateGauge builds its own column from it, so the two cannot drift) (leaders while the box is at least `minLeadersWidth` — half its height plus the callout column — else corners). **`formatCornerDelta?(delta, baseline)`** (override) words the corner block's difference line; default is `formatDelta`'s line plus the delta as a whole percent of the baseline, e.g. `+50,000 (67%)`. Curried variant: **`RateDial`** — plain numbers against zero, the form to reach for before units are decided. Exported types: `RateGaugeProps`, `RateGaugeOverrides`, `RateGaugeDataProps`, `RateGaugeDomain`, `RateGaugeBox`, `RateGaugeCalloutMode`. `geometry.ts` is otherwise private: every number the dial paints is decided there, it is pure, and it prints as a table (`geometry.test.ts`) — headless observation before the GUI. Uses `--sui-success`, `--sui-warning`, `--sui-danger`, `--sui-text-muted`, `--sui-text-secondary`, `--sui-border`, `--sui-chart-tick-color`. Use for: a "where does this stand right now" readout on a scenario card — a rate against break-even, a margin against a floor, a score against last quarter. For a one-dimensional axis whose thumb the reader DRAGS, use `BandRail`; for a bounded numeric input, use `Slider`.
  - Example:
    ```tsx
    import { createRateGauge } from "solid-ui-components";

    // Curry the wording ONCE, at your design-system layer.
    const MonthlyRateDial = createRateGauge({
      baselineLabel: "Breakeven",
      formatAgainst: (rate) =>
        rate === 0 ? "at breakeven" : `${money(rate)} ${rate > 0 ? "over" : "below"} breakeven`,
      // The sign flips: a rate that falls is payroll that rises.
      formatDelta: (delta) => `${money(delta)} ${delta < 0 ? "to" : "off"} payroll`,
    });

    // Every call site is data only.
    <MonthlyRateDial
      domain={[-30000, 30000]}
      baseline={5000}
      value={rate()}
...
## BuilderBoard
- **BuilderBoard** — Composite (Depth 2). The ONE frame a scenario builder draws its four panels in: **A** (the cashflow) over **B** (the one series this builder changes) in the top half, each an equal fill card that hands its chart a height; **C** (the controls) beside **D** (an instrument) in the bottom half, where D is held to a STATED width and C takes whatever D leaves, scrolling inside its own card when its content outgrows the half. **It owns no CSS and renders no intrinsic element** — every box is a Layout or Surface variant this library already ships (`ViewportColumn` › `HalfFillColumn` › `HalfFillColumn` × 2 › `FillCardSurface`; `HalfFillColumn` › `FillPaneRailGrid` › `FillCardSurface` × 2, with a `ScrollFillColumn` inside C) and this component only fixes the ORDER they nest in. That order is the point: three builder pages composed the same five variants three different ways and drew three different pictures of one board (a 60% pane so the gauge resized with the window; a stated 200px box so the bottom half sat at content height; an inline min-height floor). Sizes are a property of the frame, so every tab is identical by construction. Key props: `panelA`, `panelB`, `panelC`, `panelD` (each a CARD'S CONTENT; `panelD` may also be a render function `(box: Accessor<BuilderBoardPanelBox>) => JSX.Element` given D's measured content box — the rail's width by 0 until laid out — the same slot as BuilderBoardBelowChart's, for picking a gauge with `calloutModeFor(box(), labels)`; a plain element renders as before and measures nothing — a title row, then a `GrowFillBox` around a chart or a `GrowCenterColumn` around an instrument; the frame draws the card, not the title, because a title row on this board carries the page's own controls), `rail?` (the width token the rail is held to — `"gauge"`, `RateGauge`'s own derived natural width, the only token today; a pixel never crosses into a consumer). `ViewportColumn` is `height: 100%`, so the board fills a parent of DEFINITE height and collapses in a content-sized one — put it in the page's growing body region, not a plain block. **Single column on a narrow board** (Peter, 2026-09-25): the board measures its OWN width, and below `BUILDER_BOARD_SINGLE_COLUMN_BELOW` (600px) it draws one scrolling column — A and B at 240px, C at its NATURAL height (its dials keep their design height; the board scrolls, not C), D full width at the gauge's natural height — instead of viewport halves and a 292px rail that can't wrap. One breakpoint, pure: `builderBoardLayoutFor(width)` → `"split" | "stacked"` (an unmeasured board, width 0, stays split), and `observeBuilderBoard(viewport)` prints the layout and rects as a table. The root carries `data-layout="stacked"` in the single column. No CSS: the switch is measured geometry, the same pattern as BuilderBoardBelowChart.
- **BuilderBoardBelowChart** — Composite (Depth 2). The BuilderBoard for a page under the app shell's persistent chart: panel **A** is the SHELL's chart (one instance that never remounts across pages), so this board draws **B** and **C | D** only, in one of two layouts chosen by the pure `builderBoardBelowChart({ viewport, tabBarH, chartH, legendH, chartMin? = 220, bMin? = 156, gaps?: { chartToB? }, rail? })` → `{ layout, chartH, chartGaveWay, b, c, d, lowerHeight, contentHeight }` (viewport coordinates). NEITHER floor is sacrificed. **"split"** (window at least `BUILDER_BOARD_STACK_BELOW_WIDTH` = 900px wide AND the top half — the space below the tab bar less C|D and its `sm` gutter — holds chartMin + legend + the `xs` gutter + bMin): the viewport-filling board, C|D = `BUILDER_BOARD_CD_SHARE` (half) of the space below the tab bar, B the remainder; the chart takes the app's `chartH` unless that would push B under bMin, and then gives way but never below chartMin. **"stacked"** (otherwise): one column — the chart at max(chartH, chartMin), then B (bMin), C (236: a title row + five 40px rows) and D (a title row + the gauge's natural height), each full width; the page scrolls, and `contentHeight` (everything below the tab bar) sizes the app's scroller. The app renders its chart at `rects.chartH` and passes the SAME `rects` object here, so the rects the function states are the rects drawn. `observeBuilderBoardBelowChart(input)` prints the layout and rects as a text table (the headless observation). Same vocabulary as BuilderBoard: no CSS, Layout/Surface variants only (split: `ViewportColumn` › `HalfFillColumn`(B) + `NoShrinkColumn`(C|D) › `FillPaneRailGrid`; stacked: `NarrowStack` › `NoShrinkColumn` × 3, each a measured height). The B-to-C|D gutter is the board's baked `sm`, not an input. Curry with `createBuilderBoardBelowChart({ rail })`. The chart's share of the window is app policy. **Panel D's box** (additive): `panelD` may also be a render function `(box: Accessor<BuilderBoardPanelBox>) => JSX.Element` — `box()` is D's card content box (measured; the stated `rects.d` until the card is laid out, via the pure `builderBoardPanelBoxOf(measured, stated)`; the slot's type is `BuilderBoardPanelDSlot`), an accessor so a resize never remounts the instrument. For an instrument that picks its layout at a breakpoint: `panelD={(box) => <Show when={calloutModeFor(box(), labels) === "leaders"} fallback={<CornerGauge … />}><LeaderGauge … /></Show>}`. A plain element renders as before and measures nothing.
  - Factory: `createBuilderBoard({ rail })`. Exported types: `BuilderBoardProps`, `BuilderBoardOverrides`, `BuilderBoardDataProps`, `BuilderBoardRailWidth`. `geometry.ts` is deliberately private: `builderBoardRects(viewport)` derives the four rects from the gap steps and the rail token, `builderBoardTable(viewport)` prints them, and `geometry.test.ts` pins them against `Layout.css` — headless observation before the GUI. The `Builder Board` workshop bench shows the frame at three viewports beside the model's table and the measured one. Use for: a builder tab — Payroll, Hourly, Licenses — whose four regions must sit in the same four places at the same sizes on every tab. For a chart beside a rail with no top half, use `FillPaneRailGrid` directly; for a proportional split with no rail, `HalfFillColumn`.
  - Example:
    ```tsx
    import { BuilderBoard, GrowFillBox, GrowCenterColumn, TextTitle } from "solid-ui-components";

    <BuilderBoard
      panelA={<><TextTitle>Cash Flow</TextTitle><GrowFillBox><CashflowChart chartHeight="fill" … /></GrowFillBox></>}
      panelB={<><TextTitle>Pay levels</TextTitle><GrowFillBox><LevelsTimeline … /></GrowFillBox></>}
      panelC={<><SpreadRow><TextTitle>Changes</TextTitle>{controls}</SpreadRow><GrowFillBox><MutationSliders … /></GrowFillBox></>}
      panelD={<><TextTitle>Cash, on average</TextTitle><GrowCenterColumn><RateGauge … /></GrowCenterColumn></>}
    />
    ```
...
## Chart
- **Chart** — Structural (Depth 1). The SVG chart root: it owns the frame, the scales and the single pointer listener on its `<svg>`, and every mark arrives as a slot child reading `useChart()`. `onPick?: (x: number | Date, meta: ChartPickMeta) => void` makes the plot PICKABLE — a click inside the plot area reports the x under the pointer, taken from the same mapping the crosshair and `hoverX` use (the inverse of the x scale, gated on the plot area), as a `Date` when `xDomain` is date-based and a number when it is numeric. It is **not snapped**: the root does not know whether a consumer's grid is months, ISO weeks or sample indices, so snapping happens on the raw value at the call site (`LevelsTimeline.onPick` snaps to a month because a levels timeline IS a month-grained picture; this root is not). Omitted => no click listener and no cursor change at all, so a readout chart never promises an interaction it does not have; supplied => a transparent plot-sized pick surface carries a `crosshair` cursor over the PLOT only, painted first so every slot keeps its own hit-testing. Clicks in the axis gutters map to nothing and are dropped. `meta` carries `clientX` / `clientY` / `event`, so a consumer can seat a menu at the click without re-measuring the chart. Exported types: `ChartProps`, `ChartPickMeta`. Use for: click-to-add-a-date on a time chart, or click-to-select a value on a numeric one.
  - **`BarSeries` on a TIME x needs `step`.** It sizes a slot as `xs(center + step) - xs(center)`, and `step` defaults to `1` — one data unit, which suits an index x (`(_d, i) => i`, what `CompletionTimeline` passes). On a DATE domain one unit is one MILLISECOND, so the slot comes out around 1e-8 px. That is near zero but NOT zero, so the `|| plotWidth / data.length` fallback never fires and every bar renders invisible with no error. Pass the bucket's width in ms. `StackedTimelineChart`'s `columns` prop does this for you. **`BarSeries` reads the scales reactively** (its geometry is a `createMemo` over `ctx.xScale()` / `ctx.yScale()`, as `StackedAreaSeries`' has always been), so a chart that re-measures itself re-lays its bars. Computing that geometry inside the `For` row callback instead reads the scale ONCE per datum, and a self-measuring chart then keeps its first frame's fallback width for good — silently.
  - **`AreaSeries` `lower` fills between two lines (a band / channel).** `lower?: (d: T) => number` is the band's lower edge per datum. Set, the fill runs forward along `y` and back along `lower`, one closed subpath per contiguous run, and `baseline` is ignored; a NaN on either edge (with `skipMissing`, the default) breaks the band, the same rule a line follows. Omitted, the area closes on `baseline` byte-identically to before — the prop is additive. The geometry is a pure core, `buildBandPath` in `Chart/areaBand.ts` (not exported; compose the slot). No new CSS: `.sui-chart__area`'s 0.18 fill opacity makes the band visible with no styling. Not `DeviationBand`, which is a sign-coloured deviation between a series and a reference; for a neutral band use `AreaSeries lower`. `ChannelChart` composes it for a cumulative channel.
  - Example:
    ```tsx
    import { Chart, StackedAreaSeries } from "solid-ui-components";
    ...
    <Chart
      ...
      yDomain={[0, 80]}
      onPick={(at) => addChangeAt(startOfIsoWeek(at as Date))}
    >
      <StackedAreaSeries series={series()} />
    </Chart>
    ```
- **StackedAreaSeries** — Structural Primitive (Depth 1), a slot child of `Chart`. A stack of STEP-valued bands: each series is a list of `{ at, value }` points where a value takes hold at its own `at` and is held until the next one, and a series contributes nothing before its first point. `at` takes `number | Date` — the same pair the chart's reference-rule slot takes for its own value, converted in the adapter so the core stays numeric. Points are ASCENDING by `at` and values are non-negative; both are the caller's to hold, since the mark holds a value forward and a negative one would put a band below its own floor. Band *k* is closed between the cumulative top of the bands below it and its own value, so the top of the stack IS the total and two bands can never overlap — the cumulative levels are built once per plot and shared, so band *k*'s top edge and band *k+1*'s floor are the same points in the same order. A series at zero collapses its band onto that shared edge (zero area) and leaves the bands above it undisturbed. A change is crossed with a Sankey blend — flat in, cubic across, flat out, over a transition of 5% of the plot clamped to [10, 28]px and shortened wherever two changes sit closer together than a full transition apart — so a step never puts a vertical wall through a band. Props, and there are deliberately only three: `series` (`readonly StackedAreaSeriesData[]`, BOTTOM FIRST — array order is stacking order and palette order), `curve?` (`"smoothStep"` default, or `"linear"` to land each change square on its own x), `class?`. **Paint is not configurable**: band *k* takes series-palette slot *k+1* (`var(--sui-series-N)`, ADR 0003's eight-slot cap; a ninth falls back to `currentColor` rather than vanishing), painted as a translucent fill with a hairline top edge in the same hue, both fixed in `Chart.css`. A caller who could repaint one band could break the picture's only claim, which is that band *k* sits on band *k−1*. Per ADR 0010 the mark is a pure core (`Chart/stackedArea.ts`, which prints its stack as a table in `stackedArea.test.ts`) plus this one adapter, which reads `useChart()` for plot-local pixels and the x-domain to draw across; the core is not exported — a consumer composes the slot, not the geometry. It composes with every other `Chart` slot (grid, axes, a dashed reference series, crosshair) because it has no frame of its own. Exported types: `StackedAreaSeriesProps`, `StackedAreaSeriesData`, `StackedAreaPoint`, `StackedAreaCurve`. Use for: a composition of parts over time whose TOTAL is as meaningful as its parts.
  - Example:
    ```tsx
    import { Chart, StackedAreaSeries } from "solid-ui-components";

    <Chart width={640} height={240} xDomain={[0, 12]} yDomain={[0, 45]}>
      <StackedAreaSeries
        series={[
          { id: "one", label: "One", points: [{ at: 0, value: 12 }, { at: 3, value: 18 }] },
          { id: "two", label: "Two", points: [{ at: 0, value: 8 }] },
        ]}
      />
    </Chart>
    ```
...
## TreeDiffChart
- **TreeDiffChart** — Composite (Depth 2). SVG diff of two content-addressed scenario trees: the baseline root on the left, the comparison root on the right, one band per root entry between them, and a pruned `[SAME]` node per identical entry. The consumer computes the diff; the chart owns layout, edge routing and paint. **It owns no CSS and renders no intrinsic element**: the picture is `TreeDiffCanvas` (Structural Primitive, Depth 1, private to this directory), which owns `TreeDiffCanvas.css` and every SVG element, and the Composite composes it with `Tooltip` + `EllipsizedNodeLabel` (supplied into the canvas's `<foreignObject>` label slot, since a Primitive may not import a sibling Primitive), `Legend` and `TightStack`. Key props: `baseline`/`compare` (`TreeDiffRoot` — `label`, `hash`, optional `commit`, `ref`), `bands` (`TreeDiffBand[]`), `mode` (`differences` | `full`, default `differences`), `selectedId`, `onNodeClick`, `legend`. Three layouts follow the container width (narrow / compact / wide, see `frame.ts`). `selectedId` lights a pair — the clicked node, its counterpart, the chain to the spine and one level down — and dims the rest. Showcase `tree-diff-chart`.
  - **Labels never overflow the box.** A node label is consumer data of unknown length, so the title line renders as HTML in a `<foreignObject>` clamped to the node box: it ellipsizes at the box edge (via the **EllipsizedNodeLabel** Text variant) and a **Tooltip** carries the complete value. The box width is the constraint — a long label never widens it. Labels the chart itself mints (`commit`, `root tree`, `[SAME]`) are a known enumerated set and stay plain SVG `<text>`, as does the fixed-width hash line.
  - **Change kind (`kind`) is data, never inferred.** Each `TreeDiffEntry` takes an optional `kind`: `unchanged` (grey, `--sui-text-muted`), `changed` (blue, `--sui-accent`), `added` (green, `--sui-success`), `removed` (red, `--sui-danger`). It paints the node outline, every edge pointing **at** that node, and that edge's arrowhead, and emits `data-kind` plus `sui-tree-diff__node--kind-*` / `sui-tree-diff__edge--kind-*`. An arrow takes the colour of the node it points at. Purely additive: entries without `kind` paint by side exactly as before.
  - **Legend.** Composes **Legend**, keyed to the same tone tokens, showing only the kinds present in the data. Defaults on whenever any entry carries a `kind` and off otherwise; `legend={false}` suppresses it without changing the paint.
  - **createTreeDiffChart factory** — `createTreeDiffChart({ legend })` returns a curried `Component<TreeDiffChartDataProps>`. `legend` is the ONLY presentational prop the chart has: `baseline`, `compare`, `bands`, `selectedId` and `onNodeClick` are data or callbacks, `mode` included — "show me only the differences" is a question the consumer's UI asks, not a look the design system picks — and the chart mints no label text of its own beyond an enumerated set, so there is no formatter to curry. Curried variant: **`ScenarioTreeDiff`** — the zero-config drop-in, data and callbacks only. It leaves `legend` at the chart's data-derived default ON PURPOSE rather than pinning it: a pinned `true` would key an empty legend for entries carrying no `kind`, and a pinned `false` would hide the key from the consumers who do. A consumer that keys the colours in its own chrome curries `createTreeDiffChart({ legend: false })` once in their own layer. Exported types: `TreeDiffChartOverrides`, `TreeDiffChartDataProps`.
  - Exported types: `TreeDiffChartProps`, `TreeDiffBand`, `TreeDiffChild`, `TreeDiffEntry`, `TreeDiffRoot`, `TreeDiffMode`, `TreeDiffSide`, `TreeDiffKind`. **`TreeDiffChart`, `TreeDiffChartProps`, `TreeDiffRoot`, `TreeDiffBand`, `TreeDiffChild`, `TreeDiffEntry` and `TreeDiffMode` are a consumer contract** (thorcasting-ui adapters) pinned at the package root by `src/components/TreeDiffChart/barrel.test.ts` — rename one and a test fails instead of a consumer's build. Exported pure helpers: `KINDS`, `kindColor`, `kindLabel`, `kindLegendItems`, `presentKinds` — the change-kind vocabulary a consumer needs to supply `kind` and to key the same tokens in its own chrome. **The layout and frame geometry exports are PRIVATE** (`computeTreeDiffLayout`, `computeFrame`, the three width thresholds, `TreeDiffLayoutInput`, `TreeDiffLayout`, `TreeDiffLayoutMode`, `Frame` and the five `Layout*` node/edge/band/guide/caption types): audited 2026-09-17 with no caller anywhere — not a consumer repo, not the showcase, not the (since retired) Scenario Board bench — and they contradicted this release's own policy, under which `RateGauge`'s and `MutationSliders`' geometry stayed private and `LevelsTimeline` documents reaching `./geometry` directly for a dev surface. Published in 0.171.0, deprecated in 0.172.0 and **removed from the barrel on 2026-09-18** (add/deprecate/delete phase 3), re-verified against all 18 consumer repos on this machine first. **There is no public successor and none is needed**: `package.json` `exports` publishes the root, the themes and the CSS only, so a package consumer never had a route to these modules; the relative import is for an in-repo dev surface, exactly as the workshop's board benches reach `LevelsTimeline/geometry`. See `docs/adherence/variant-audit-2026-09-16.md`. Uses `--sui-accent`, `--sui-success`, `--sui-danger`, `--sui-text-muted`, `--sui-text-primary`, `--sui-text-secondary`, `--sui-border`, `--sui-border-bright`, `--sui-bg-elevated`, `--sui-font-mono`. Use for: scenario/config tree comparison, content-addressed build or commit diffs, any two-sided structural diff that needs a per-node change key.
...
## ScrubChart
- **ScrubChart** — Composite (Depth 2). Composes `DateAxis` (Atomic) + a user-supplied chart slot. Renders the chart at **linear** scale (every cell the same pixel pitch — `width / cells.length`) and draws a translucent **window** rect over the slice of cells currently visible in the DateAxis viewport. Classic overview + detail: the chart is the minimap, the axis is the zoomed-in detail view. Generic over `C extends Cell`; consumers attach payload to each cell and read it back inside `renderChart`. Scrubbing supports axis-cell click and drag-on-chart; linear pitch means each pointer move maps directly to one cell — no anchored layout, no tween. The axis auto-scrolls to keep the selected cell centred, so the window-band tracks the selection. Key props: `cells: C[]`, `selected: number`, `onScrub: (index, cell) => void`, `renderChart: (ctx) => JSX.Element`, `renderCell` (forwarded to the inner DateAxis), `chartHeight?` (default `200`; also accepts **`"fill"`**, which makes the frame take its CONTAINER's height — the chart measures its own frame and drives the plot span, the axis rows and the `viewBox` from that. Additive: the numeric path is untouched. Two conditions: the container must HAVE a definite height, because the frame becomes `height:100%` and the svg inside is itself `height:100%`, so against an indefinite parent the frame collapses; and `chartHeightExpanded` is ignored, since moving the frame between two pixel heights says nothing when the container owns the height — the chevron does not render and the tween never runs), `cellWidth?` (default `40`, the axis cell width), `today?`, `showGridlines?` (opt-in, default `false` — draws a horizontal rule across the plot at every y-axis tick, the same shape `Chart`'s `Grid` slot draws: solid `--sui-border`, 1px, never dashed. The rules use the SAME tick set as the y-axis labels, so a line never sits where no label is, and they paint BENEATH the `renderChart` series. Horizontal only; no effect without `yDomain`), `highlights?` (opt-in, default none — `ScrubChartHighlight[]`, one shaded band per entry: `{ from, to, class? }` where `from` and `to` are CELL INDICES, both ends INCLUSIVE and covering the whole cell. Bands paint in the BOTTOM layer of the frame — beneath the gridlines and beneath the `renderChart` series — so a band backs the data instead of dimming it, unlike the window band above it. ScrubChart clamps both ends to the cell range and swaps them when `from > to`, so a band computed from live data cannot draw outside the plot. Bands render in array order, so a later band paints over an earlier one. Style one band through its own `class` — it always wins, because the faint neutral default rides on the rect as a presentation attribute rather than as a rule on the shared base class `.sui-scrub-chart__highlight`, so stylesheet order cannot decide the colour. Themes move the default through `--sui-scrub-chart-highlight-fill` and `--sui-scrub-chart-highlight-opacity`. Vertical extent is always the full plot — for a band bounded in Y see `CashflowBalanceSeries.fill`). `yFitDomain?` (opt-in, default none — `(from, to) => [number, number] | null`, the Y-DOMAIN of a cell range in data units. ScrubChart calls it with the VISIBLE window in `"visible"` mode and with the WHOLE cell range (`[0, cells.length - 1]`) in `"series"` mode; both ends are INCLUSIVE cell indices, matching `ScrubChartHighlight`. The callback exists because `renderChart` is a slot — the chart never sees the values, so the caller states the extent, and one array could not serve a chart that draws two series. Return the RAW extent: ScrubChart pads it by `yFitMargin`, snaps it with `nice()`, then applies `yFitPin`. Returning `null` falls back to `yDomain`. Precedence for the ONE effective domain that drives the ticks, the labels, `yToPlot`, `yAxisWidth` and `showGridlines`: `yFitDomain` when set and non-null, else `yDomain`, else no y-axis. Setting this prop also renders the fit control — ONE small icon button in the chart's AXIS ORIGIN CORNER: in the y-axis label column, left of `plotLeft`, and CENTRED on the x-axis tick labels. It is a plain `<button>`, not `Button`: at rest it is the glyph on a plain SCRIM, with no border and no ring, and it strokes in `--sui-accent` — the same token the default series line takes, so the control names the line it rescales. Hover draws a small rounded border INSIDE the button, 20px against the 26px hit target, because a pseudo-element carries it; the target itself stays 26px for a pointer and for a keyboard. `:focus-visible` keeps an outline, so the affordance is not hover-only. The SCRIM is there at rest, painted by that same pseudo-element in `--sui-bg-base` falling back to `--sui-bg-elevated`, so the glyph reads over whatever the button covers; hover stays the stronger state. Both corner controls paint at `z-index: 2`, above every layer of the frame — the highlight bands, the gridlines, the `renderChart` series, the axis tick labels, the window band and the pan overlay — so an overlap always resolves in the button's favour. It reserves NO row of its own. The x-axis row grows from its 22px label height to the button's 28px footprint, which costs the plot 6px. The DEFAULT `yAxisWidth` grows to 38px — that footprint plus a 10px GUTTER. The gutter moves the y tick labels right, which leaves the button clear of them; the frame hides its overflow, so the button cannot move left instead. An explicit `yAxisWidth` is used as given and a narrow one clips the control. With `xTickCadence="none"` there is no label row, so the footprint IS the row. Levelling the button on the x labels lifts its top edge a few px ABOVE `plotBottom`, where the lowest y tick sits. ScrubChart therefore BOTTOM-ALIGNS the lowest y label on its own gridline instead of centring it there, so the label's whole line box clears the button; the gridline and the tick stub do not move. The same clamp holds a label off the frame's top edge, and the two bounds cannot fight — they cross only in a frame shorter than one label, and the label then centres in the room it has. The button covers no data at a usual width and never intercepts the pan gesture. Below roughly a 160px frame a corner button does land on an x tick label; it floats over that label on its scrim, and it neither reserves space, nor moves, nor hides. The button shows the ACTION a click performs, not the current mode: under `"visible"` it draws `zoom-out` and names itself "Fit to all", and under `"series"` it draws `zoom-in` and names itself "Fit to visible". It carries no `aria-pressed` — this is a switch between two named modes, not a pressed toggle), `yFitMargin?` (default `0.08` — fraction of the fitted extent added above and below a FREE end. A pinned end takes no margin), `yFitPin?` (opt-in, default none — `{ min?, max?, visible?: { min?, max? }, series?: { min?, max? } }`. A pinned end ignores the fitted extent, the margin and the snap, and renders EXACTLY as given; the free end still gets all three, so the ticks read 0, 20, 40 rather than 0, 17.3, 34.6. `min` / `max` apply to BOTH modes and a mode key overrides them for that mode only, resolved as `yFitPin[mode]?.min ?? yFitPin.min`. Two pinned ends are used as given, with no padding and no snap. The pin is a PROP rather than the caller's job because the callback returns a raw extent that ScrubChart pads afterwards — a caller who returned 0 as its min would watch the margin push it below zero), `yFitBounds?` (opt-in, default none — `Partial<Record<"visible" | "series", { min?, max? }>>`, one entry per y-scale mode. A bound only WIDENS the fitted domain: the low end takes `Math.min(fitted min, min)` and the high end takes `Math.max(fitted max, max)`. Data outside the bound therefore stays on the plot, which is what separates it from `yFitPin` — a pin OVERRIDES an edge and clips that data away. A series of 300..900 bounded at `{ min: 0 }` draws 0..900, and the same bound on a series of -50..900 draws -50..900. The bound applies LAST, after `yFitMargin` and after the `nice()` snap, so a bound of 0 puts the floor on exactly zero while the opposite end keeps its snap. A mode with no entry keeps the domain it fitted), `yScaleMode?` / `onYScaleModeChange?` (`"visible" | "series"` — controlled mode. Omit `yScaleMode` and ScrubChart owns the signal, starting at `"visible"`). Y-AXIS MODE: `yAxisMode?` / `onYAxisModeChange?` (opt-in, default none — `ScrubChartYAxisMode` = `"auto" | "fixed" | "autoscale"`, CONTROLLED, and the master switch for a three-segment switch — Auto | Fixed | Fit, a curried `SegmentedControl` at `size="xs"`, each segment a word with a Tooltip stating what the mode does — in the axis ORIGIN corner. It REPLACES the y-fit button there when both are set, under the same guarantees: the x-axis row is the 28px corner footprint, the DEFAULT y-axis column grows to `Y_AXIS_MODE_COLUMN` (96px: the 84px switch + inset + gutter), and the lowest y label is lifted clear of the corner. ScrubChart only SHOWS the mode and reports a pick; what each mode does to the domain is the caller's policy, stated through `yDomain` / `yFitDomain`). RANGE EDITOR: `onYRangeChange?` (opt-in — `(range: ScrubChartYRange) => void`, `{ min, max }` in data units) + `yRangeField?` (`"number"` default, or `"currency-cents"` to show dollars and emit cents). In FIXED mode (`yAxisMode="fixed"`) with the callback set, the y-axis label column becomes a button (`.sui-scrub-chart__y-axis-hit`, from `plotTop` down to the corner control) and a click opens `ScrubChartYRangeEditor` over the plot's top-left: Max above Min, seeded from the domain on screen, Cancel / Apply, Enter applies, Escape cancels, stops propagation and calls `preventDefault` (a `FullscreenBox` around the chart ignores an Escape already handled, so it stays open); Apply is disabled with a reason while min is not below max. The chart does not hold the range — the caller stores it and passes it back through `yDomain`. Leaving fixed mode closes the editor. The draft policy is the pure `draftYRange` (`yRange.ts`), printable with `yRangeDraftTable`. EXPAND: `chartHeightExpanded?` (opt-in, default none — the height in px the chart grows to, and THE MASTER SWITCH for the expand control. Setting it draws ONE small chevron in the bottom-RIGHT corner of the frame, opposite the y-fit button, and a click moves the frame between `chartHeight` (the collapsed height) and this one. Leave it unset and the chart keeps `chartHeight` and draws no chevron. The chevron mirrors the y-fit button exactly: same 26px hit target, same 2px inset, same glyph and scrim in `--sui-accent`, same level, same hover border and focus ring, and the same line on the x-axis row, because both take the shared classes `.sui-scrub-chart__corner` and `.sui-scrub-chart__corner-btn`. Only the edge differs, so the two coexist on one chart. Each button also carries a name of its own — `.sui-scrub-chart__y-fit-btn` and `.sui-scrub-chart__expand-btn` — as the hook a consumer overrides one of them with. The chevron shows the STATE and the direction a click moves the frame: `chevron-down` collapsed, named "Expand chart", and `chevron-up` expanded, named "Collapse chart". Like the y-fit button it reserves no row of its own; the x-axis row grows to the same 28px footprint), `expanded?` / `onExpandedChange?` (`boolean` — controlled state. Omit `expanded` and ScrubChart owns the signal, starting COLLAPSED, and `onExpandedChange` reports every click either way), `expandTransition?` (default `240` ms, `false` jumps — the time the frame takes to reach the other height. ONE height accessor drives the plot span, the axis rows, the window band and the `viewBox` together, so the whole chart eases with the frame rather than jumping inside it. The easing is the same exponential approach the fitted y-domain takes, and a reader who sets `prefers-reduced-motion: reduce` gets the new height at once. No effect without `chartHeightExpanded`, so a caller that moves `chartHeight` itself keeps the jump it has always had). TOP ACTION: `topAction?` (opt-in, default none — `true | ScrubChartTopAction`, and THE MASTER SWITCH for the THIRD corner control. `true` draws ONE small button in the TOP-RIGHT corner of the frame with the default glyph and name — a `minus` glyph named "Minimize chart" — and a click replaces the WHOLE chart, frame and ribbon alike, with the minimized bar. An object states `{ icon?, label?, onClick? }` and changes any of the three; `topAction={{}}` asks for the same default button as `topAction` alone. An `onClick` of the caller's own REPLACES the minimize step: the button calls it, the chart never leaves the frame, and the page owns what follows. The button joins the corner family whole — same 26px hit target, same 2px inset, same glyph and scrim in `--sui-accent`, same hover border and focus ring, same `z-index: 2` — because it takes the shared classes `.sui-scrub-chart__corner` and `.sui-scrub-chart__corner-btn`, plus `.sui-scrub-chart__top-action` and `.sui-scrub-chart__top-action-btn` as the hooks a consumer overrides it with. Two things separate it from the bottom pair. It is named for its CORNER and not for its function, because a caller re-aims it and a name like `__minimize` would then lie. And it has NO axis gutter to sit in, so it floats over the top right of the plot on its scrim, reserves no row, and hides for nothing — the same tradeoff the bottom pair already takes below roughly a 160px frame), `minimized?` / `onMinimizedChange?` (`boolean` — controlled state. Omit `minimized` and ScrubChart owns the signal, starting OPEN, and `onMinimizedChange` reports every click either way. `minimized` with no `topAction` is IGNORED: nothing could raise the bar, so nothing may strand a reader in one. Minimized is its OWN axis and not a third step under `expanded` — the height signals are untouched, so a chart minimized while expanded comes back expanded, and the expand chevron does not render while the bar is up because there is no frame for it to move), `renderMinimized?` (opt-in, default none — `(ctx: ScrubChartMinimizedContext<C>) => JSX.Element`, the bar's content left of the restore button. With no slot the bar prints the cell range's DATE SPAN — `"Sep 22 – Dec 31"`, or `"Sep 22 '26 – Mar 22 '27"` when the two ends fall in different years — which is all ScrubChart can derive, because `renderChart` is a slot and the component never reads a value. `ctx` carries `cells`, `selected` and that same `summary`, so a caller prints its own value beside the span rather than re-deriving it. The bar is a curried `Row` and a curried `Text`, so it owns no flex rules and takes the library's one gap scale; the RESTORE button is fixed at a `plus` glyph named "Restore chart" and reuses `.sui-scrub-chart__corner-btn`, so the glyph a reader clicks to come back matches the one they clicked to leave. `.sui-scrub-chart__minimized` and `.sui-scrub-chart__minimized-summary` are the consumer hooks. In `"fill"` mode the root DROPS its fill modifier while the bar is up, so one line does not stretch down the container). The `ScrubChartContext` passed to `renderChart` exposes `cellToX(i)`, `cellBounds(i)`, `dayPitch`, `selected`, `cells`, `windowCells` (`[firstIdx, lastIdx]`), `windowBounds` (`[leftX, rightX]`), `width`, `height`. `ScrubChartReferenceLine` draws one horizontal reference rule inside `renderChart` / `renderChartOverlay` — the `ScrubChart` adapter for the reference-line mark (ADR 0010), taking `ctx: ScrubChartContext<C>` as a prop since `ScrubChart` has no Solid context to read. `CashflowScrubChart` uses it for its zero line and its `horizontalMarkers`. `ScrubChartBand` is the `ScrubChart` adapter for the deviation-band mark (ADR 0010, dside task 45165): same `ctx` convention, calls the shared `buildDeviationBand` core, and carries no colour default of its own (`positiveClass` / `negativeClass` / `positiveFill` / `negativeFill` are plain per-sign props) — `Chart`'s own slot-child adapter for the same core is `DeviationBand`. `CashflowScrubChart` uses `ScrubChartBand` for its green-above / red-below deviation shading, applying that reading at its own call site. `ScrubChartLabels` is the `ScrubChart` adapter for the label-ladder mark (ADR 0010, dside task 45164): same `ctx` convention, calls the shared `placeLabels` / `drawnLabels` core (`Chart/labelPlacement.ts`) to decide where each label lands and pair it with its text; `Chart`'s own slot-child adapter for the same core is `ChartLabels`, which reads `useChart()` for plot-local pixels instead. `CashflowScrubChart` builds its own cashflow-bound candidates (`labelCandidates.ts`) and hands them to `ScrubChartLabels`. `ScrubChartCrosshair` is the `ScrubChart` adapter for the hover-crosshair mark (ADR 0010, dside task 45169): same `ctx` convention, calls the shared `buildCrosshair` core (`Chart/crosshairMark.ts`) — index-addressed, so every dot shares the guide's own x, unlike `Chart`'s `Crosshair` (the other adapter for the same core), which finds each series' nearest point independently and can drift from it. `ScrubChartTooltip` is the `ScrubChart` adapter for the tooltip-flip mark: calls the shared `placeTooltipX` core (`Chart/tooltipPlacement.ts`) off its own MEASURED width (a ref + `observeSize`), the same technique `Chart`'s `ChartTooltip` uses for the same core. `CashflowScrubChart`'s hover readout composes both. Exported types: `ScrubChartProps`, `ScrubChartContext`, `ScrubChartHighlight`, `ScrubChartOverrides`, `ScrubChartDataProps`, `ScrubChartTopAction`, `ScrubChartMinimizedContext`, `ScrubChartYScaleMode`, `ScrubChartYAxisMode`, `ScrubChartYRange`, `ScrubChartYFitPin`, `ScrubChartYFitBound`, `ScrubChartReferenceLineProps`, `ScrubChartBandProps`, `ScrubChartLabelsProps`, `ScrubChartCrosshairProps`, `ScrubChartCrosshairSeries`, `ScrubChartTooltipProps`. Theme tokens: `--sui-accent`, `--sui-border`, `--sui-bg-elevated`, `--sui-bg-base`, `--sui-radius-md`, plus override CSS variables `--sui-scrub-chart-window-fill` and `--sui-scrub-chart-window-stroke` for the window band's colours. Use for: linked chart + axis pairings where the chart needs to show the big picture while the axis is the user's zoomed-in detail view. **`chrome?: "own" | "frame"`** (override, default `"own"` — today's rendering, unchanged): `"frame"` draws NONE of the chart's own controls — no y-axis-mode switch or y-fit button in the origin corner (and no corner footprint or column: the plot takes the room), no fixed-mode range editor, no expand chevron, no top action — because a `ChartFrame` above holds them; the chart still follows `yDomain` / `yFitDomain`, `yAxisMode` and `expanded` as controlled props. Curry it (`createScrubChart({ chrome: "frame" })`); the shipped curry is `FramedCashflowScrubChart`.
  - Example:
    ```tsx
...
## CashflowScrubChart
- **CashflowScrubChart** — Domain Composite (Depth 3). Composes `ScrubChart` (Depth 2) with a baked-in cashflow day-cell renderer (date corner + diverging green/red bar + dollar amount) and a baked-in running-balance line drawing. Zero-config at the call site: just pass `cells: CashflowCell[]` + `selected` + `onScrub`. Per-cell payload `CashflowCell = Cell & { cashflowCents: number; balanceCents: number }`: `cashflowCents` is the day's net flow (negative for an expense day); `balanceCents` is the cumulative running balance. The bar's fill fraction is hand-tuned for typical cashflow magnitudes (saturates at $2,200/day-equivalent); the line plots `balanceCents` across all cells. Inherits ScrubChart's window-band overlay, axis auto-scroll, and pointer scrub behaviour. Key props: `cells: CashflowCell[]`, `selected: number`, `onScrub: (index, cell) => void`, `today?`, `chartHeight?` (default `200`; **`"fill"`** forwarded to `ScrubChart.chartHeight` unchanged — see its doc for the two conditions), `cellWidth?` (default `60` — matches the cashflow cell's content; not `40` like bare ScrubChart), `lineClass?` (CSS class added to every mark of the PRIMARY balance line alongside each mark's base class — the polyline AND the primary dot on the hover crosshair; the counterpart of `CashflowBalanceSeries.class`, which reaches the same set of marks for an overlay series; colour / dash / opacity are the consumer's. One class per line across every mark is what lets a consumer hide a whole line: a class that stopped at the polyline left the crosshair dot behind, visible and matching nothing in the tooltip), `showGridlines?` (opt-in, default `false` — forwards to `ScrubChart.showGridlines`: a dim horizontal rule at every y-axis tick, undashed because every short dash pattern on this chart already means another line type), `highlights?` (forwards to `ScrubChart.highlights` unchanged — shaded bands over day ranges, drawn beneath the balance line and every overlay series. The shape stays `ScrubChartHighlight` rather than gaining a cashflow alias, because a band carries no cashflow vocabulary). Y-FIT: `yFitDomain?`, `yFitMargin?`, `yFitBounds?`, `yFitTransition?`, `yScaleMode?`, `onYScaleModeChange?` and `yAxisWidth?` forward to `ScrubChart` unchanged, and each takes its type from `ScrubChartProps` so the two cannot drift. `yAxisMode?` / `onYAxisModeChange?` forward the same way (the Auto | Fixed | Fit origin-corner switch — see ScrubChart). `onYRangeChange?` forwards too, with the editor set to show DOLLARS and emit CENTS — store the range and pass it back as `yMin`/`yMax`. OUT-OF-RANGE MARKERS: whenever the domain clips drawn data — a fixed `yMin`/`yMax` narrower than the data, or a line-based domain under a wider range cone — the chart marks ONE value per clipped edge: the global PEAK above the plot top (upward chevron + compact value, `.sui-cashflow-scrub-chart__overtop`; always, as before) and — in FIXED mode only (`yAxisMode="fixed"`), since a consumer's floored or fitted domain may clip a cone's lower edge on purpose — the global TROUGH below the plot bottom (downward chevron + value, same classes plus `--bottom`). The candidates are the primary line, every balance series, and every fill BASELINE (a cone's lower edge). The decision is the pure `outOfRange` core (`outOfRange.ts`), printable with `outOfRangeTable`. Setting `yFitDomain(from, to)` — the balance extent of an INCLUSIVE cell range, in cents — draws the fit toggle in the chart's origin corner, which switches the y-axis between fitting the visible window and fitting the whole series. The fitted domain OUTRANKS `yMin` / `yMax` / `yPadFraction`: those three still compute a domain and it stays the FALLBACK, applying whenever the callback is absent or returns `null`, so a caller that sets none of the six keeps the chart it had. `yAxisWidth?` is forwarded for one reason worth stating: ScrubChart auto-measures the y-axis column and, with `yFitDomain` set, widens that default until the toggle button fits — a host that cannot state the width cannot correct a clipped button. `yFitBounds?` states the edges the fitted domain always includes, per mode, in cents: `{ series: { min: 0 } }` holds the zero line on the axis while every day that dips below it stays on the plot, because a bound widens the domain and never narrows it. `yFitPin` is NOT forwarded yet. EXPAND: `chartHeightExpanded?`, `expanded?`, `onExpandedChange?` and `expandTransition?` forward to `ScrubChart` unchanged as well, each typed through an indexed access. Setting `chartHeightExpanded` draws the expand chevron in the bottom-right corner of the frame, opposite the y-fit toggle, and a click moves the chart between `chartHeight` and that height over `expandTransition` ms. A host that draws every chart through this wrapper therefore needs no expand button of its own. Omit `expanded` and the chart owns the state, starting collapsed; a caller that sets no `chartHeightExpanded` keeps the height and the chrome it had. TOP ACTION: `topAction?`, `minimized?`, `onMinimizedChange?` and `renderMinimized?` forward to `ScrubChart` unchanged, each typed through an indexed access. Setting `topAction` draws the minimize button in the TOP-RIGHT corner of the frame, and a click replaces the whole chart with a one-line bar. This wrapper supplies the bar's DEFAULT LINE, which `ScrubChart` cannot: it reads the balances, so the bar states the CLOSING balance beside the date span — `"$148,204 · Sep 22 – Mar 22"`, from `minimizedCashflowLine`. The closing balance and not the opening one, and not the selected day's: the chart is a forecast, the number a reader wants from a folded forecast is where it ends up, and a selected day would make the bar move under a scrub the reader can no longer see. A `renderMinimized` of the caller's own replaces that line, and `ctx.summary` still hands back the span. A host that draws every chart through this wrapper therefore needs no minimize button of its own; a caller that sets no `topAction` keeps the chart and the chrome it had. Owns `CashflowScrubChart.css`; allowed at Depth 3 because the diverging-polarity visual is genuinely domain-specific and not expressible as variants of upstream atomics. Theme tokens: `--sui-cashflow-positive`, `--sui-cashflow-negative` (bar + amount colour); `--sui-cashflow-cell-positive-bg`, `--sui-cashflow-cell-negative-bg` (cell background tint); inherits `--sui-scrub-chart-window-fill`, `--sui-scrub-chart-window-stroke` from `ScrubChart`. LINE AND MARKER LABELS: a `CashflowBalanceSeries` with a `label` draws that label on the chart, and a `CashflowChartMarker` does too once it names a zone. `labelPlacement?: CashflowLabelZone` (`"auto" | "body" | "right" | "below"`, default `"auto"`) states a PREFERENCE, not a lock — the chart walks body → right → below and takes the first zone the measured text fits, and a label that fits nowhere is dropped in silence (nothing is logged). The zone rides on the DATA rather than on a curried variant because one chart routinely needs different zones for different lines; a variant would force them all into one. Only an EXPLICIT zone buys frame space: `"right"` widens a gutter past the plot, `"below"` adds a row under the x-axis ticks (two rows at most), and an `"auto"` label uses whatever another label bought but never creates space itself — so a chart with no labels, or with `"auto"` labels alone, keeps every pixel it had. **The right gutter is capped** at `MAX_RIGHT_GUTTER_SHARE` (25%) of the plot it leaves, measured against the frame's width: past that (a long label on a phone), every `"right"` label falls back to `"below"`, in the reservation and the placement alike, so a narrow chart keeps its plot. Pure `rightGutterFits(labels, chartWidth)` + `demoteRightLabels(items)` (internal `Chart/labelPlacement`); an unmeasured frame keeps the gutter. ScrubChart reports its measured frame width through **`onChartWidthChange?(width)`** for exactly this. Exported types: `CashflowCell`, `CashflowScrubChartProps`, `CashflowBalanceSeries`, `CashflowSeriesFill`, `CashflowChartMarker`, `CashflowLabelZone`. Use for: cashflow / coffers visualisations that need both per-day detail (the axis ribbon) and the running-balance arc (the chart).
  - Factory: `createCashflowScrubChart({ yFitTransition, expandTransition, chrome })` — MOTION is the one presentational thing this chart has to freeze; every other prop is data or sizing. Curried variant: **`StillCashflowScrubChart`** — the chart that draws its FINAL FRAME AT ONCE on mount and on every data change (`yFitTransition: false`, `expandTransition: false`). The default chart eases its fitted y-axis toward each new domain over 240ms, which a reader who pans wants and a reader who switches TABS does not: the chart mounts on one series, the page's fold lands a frame later, and the glide hides whether anything changed. Exported types: `CashflowScrubChartOverrides`, `CashflowScrubChartDataProps`.
  - **`FramedCashflowScrubChart`** — the chart inside a `ChartFrame`: `chrome: "frame"` (see ScrubChart) plus Still's motion (`yFitTransition: false`, `expandTransition: false`). It draws no Auto | Fixed | Fit switch, no range editor, no expand chevron and no top action; the frame's split button and `createYAxisStrategy` own the y-axis, and the chart takes the result as controlled props — `yMin` / `yMax` (the strategy's domain, in cents), `yAxisMode` (still marks out-of-range values in `"fixed"`) and `expanded`. Example: `<ChartFrame yAxisMode={axis.mode()} onYAxisModeChange={axis.setMode} onYAxisPress={axis.press} …><FramedCashflowScrubChart cells={cells()} chartHeight="fill" yAxisMode={axis.mode()} yMin={axis.domain()?.[0]} yMax={axis.domain()?.[1]} /></ChartFrame>`.
  - Example:
    ```tsx
...
- **LevelsTimeline** — Atomic (Depth 1). Owns `LevelsTimeline.css`; composes no other component. A horizontal time chart of **levels**: each level is a RAIL at a fixed y — `value` IS its y, on a real value axis — and what varies along it is its THICKNESS, which is the COUNT holding that level. A count moving between two levels is not a step in either rail: it is a Sankey-style FLOW, a ribbon leaving one band's cap and arriving at another's on the same width scale, so the lower rail visibly thins as the upper one thickens. One-ended flows are conservation made visible — `from` only is a DEPARTURE (fades out), `to` only an ARRIVAL (fades in) — so no rail ever thins silently. A rail nothing happened to is bridged by a CONTINUATION painted exactly like the band, so a join the chart made for its own reasons never reads as an event.
  ...
  - **Dated events (2026-09-24).** Flags are NUMBERED 1…n in time order by the chart itself, in one fixed box that fits two digits; `Mutation.label` is announced, never painted. Flags whose boxes would overlap NUDGE sideways just enough to clear (`nudgeFlagCentres`), keeping their rule at the true x with a short leader. Hovering or focusing a flag shows the exact day and the optional `Mutation.details` lines. With `onMoveMutation?(id, at)` flags DRAG along x (and move a day with the arrow keys), snapped to the day and clamped between neighbours (previous + 1 day … next − 1 day), so numbering never changes under the pointer; a press that barely moves is still a click. The bottom axis is DATED: a tick at every flag's exact day, cadence ticks by span (≤ ~4 months weekly on ISO Mondays UTC, ≤ 15 months monthly, ≤ 35 quarterly, else yearly), and horizontal labels painted only where they never overlap, flag dates first, abbreviated by **`abbreviateDates`** (`YYYY-MM-DD` first and at a year change, `MM-DD` otherwise) — exported so a consumer's change tabs (`MutationToolbar` chips) use the same rule.
  - **Click strategy.** `pickAt?: PickStrategy` decides what `onPick` reports and where the hover crosshair sits. **`pickDay`** picks the whole UTC day under the pointer, clamped to the domain. It is behaviour, so it is CURRIED (`createLevelsTimeline({ formatValue, pickAt: pickDay })`), never passed at a call site. Without one the chart keeps its old nearest-MONTH pick (deprecated as a default — pass a strategy).
  ...
  - Key props: `levels` (`readonly Level[]` — `id`, `label`, `value`, `points`), `transfers?`, `mutations`, `domain` (`TimeDomain`, the consumer's visible span, never derived), `valueDomain?` (pin the y range instead of deriving it, so rails hold still while one value moves; a level outside it clamps rather than widening it — a DERIVED range is nicened out to whole ticks, a PINNED one is left exactly as given), `selectedMutationId?`, `onSelectMutation?`, `formatValue?`, `onPick?` (a click reports the date the pick strategy resolves — the month under the pointer when none is curried; turning that into anything is the consumer's business, and a flag click never fires it), `onMoveMutation?` (flags become draggable).
  ...
  - Factory: `createLevelsTimeline({ formatValue, pickAt })` — the format (presentation) and the pick strategy (behaviour) curry once at the consumer's design-system layer. Curried variant: **`LevelsRailChart`** — plain numbers, the form to reach for before units are decided. Exported: `LevelsTimeline` (the base component, kept published for live consumers), `LevelsTimelineProps`, `LevelsTimelineOverrides`, `LevelsTimelineDataProps`, the data types `Level`, `CountPoint`, `Transfer`, `Mutation`, `TimeDomain`, `TimeValue`, `timeOf` (the only way to compare two `TimeValue`s, which are `Date | number`), `pickDay` + `PickStrategy`, `abbreviateDates`, and `levelsValueFit` (the rails' value extent — the fit a held axis tracks: compose `createAxisWaterMarks(() => levelsValueFit(levels))` with `valueDomain` and a shrink `IconOnlyButton` for an axis that expands on its own and shrinks on request). `geometry.ts` is deliberately private: every number the chart paints is decided there, it is pure, and it prints as a table (`geometry.test.ts`) — headless observation before the GUI. Uses `--sui-accent`, `--sui-border`, `--sui-text-primary`, `--sui-text-muted`, `--sui-bg-elevated`. Use for: a ladder of numeric levels whose POPULATION moves over time — a pay ladder, a tier of accounts, a grade band. For one value's own trajectory use `CashflowScrubChart` or `BurndownChart`; for a value against a reference right now, `RateGauge`.
  - Example:
    ```tsx
...
## MarkedSlider
- **MarkedSlider** — Atomic Primitive (Depth 1). ONE vertical slider, marked: a thin track line running the whole `domain`, a shaded **allowed range** box on it (`range`, which is the clamp as well as the drawing), a muted **prior** arrowhead and an accent **value** arrowhead pointing at the track from opposite sides so a pair at the same figure meets nose to nose, a wider **change line** toned `raise`/`cut` between them, and the caller's `deltaLabel` as a figure level with that line's midpoint. Hue is never the only cue — the value above the prior says the same thing by position. **Named by shape, not by domain**: there is no entity here and no scenario, only a domain, a range, a value, a prior value and a string the CALLER formatted — this component never formats a number, because the unit is not its business. Wraps the headless Kobalte slider (`@kobalte/core/slider`), which is what a Primitive is allowed to do (the dependency is external, like the DOM), and it is the only place in this stack that draws SVG or owns a stylesheet: `MarkedSlider.css` is its own interior geometry — a canvas, an overlay lying 1:1 on it, a percentage-placed thumb — the same disposition as `Slider`'s notches. **THE RANGE IS THE CLAMP**: both values are pulled onto it before they are drawn, the thumb announces the clamped figure through `aria-valuemin`/`aria-valuemax`/`aria-valuetext`, and neither callback ever emits outside it, so the thumb stops dead at an edge. Key props: `domain` (`[min, max]`, the track's whole extent), `range` (`[min, max]`, the allowed span), `value` (`number | null` — `null` is "no value at all": no thumb, disabled, `data-removed`, and the range box and prior mark stay so the comparison survives), `prior` (`number | null`), `height?` (px; the viewBox is 1:1 with it, so text does not stretch), `snap?` (the grid emitted values land on), `dragStep?` / `keyStep?` (derived from the domain when omitted — a drag is continuous and a key press is not, because Kobalte's `step` governs both), `deltaLabel?` (already formatted; omitted, the slot is rendered and hidden so the shape never changes), `label` (the thumb's accessible name), `valueText?` (what a screen reader announces), `active?` (draws the selected ring), `onChange?` (every intermediate value of a drag and every key step), `onChangeEnd?` (once per committed gesture), `onMeasure?` (the box was measured — deliver the first size synchronously and the drawing and the pointer agree on frame one). Exported types: `MarkedSliderProps`, `MarkedSliderOverrides`, `MarkedSliderDataProps`. Uses `--sui-success`, `--sui-danger`, `--sui-accent`, `--sui-accent-rgb`, `--sui-text-secondary`, `--sui-border`, `--sui-font-mono`. Use for: one bounded quantity whose ALLOWANCE and whose PREVIOUS figure both have to be visible while it is dragged. For a plain bounded value use `Slider`; for a value against a reference use `RateGauge`; for a value axis carrying named thresholds use `BandRail`; for a whole ROW of these with names, readouts and pinning use `MutationSliders`.
  - Factory: `createMarkedSlider({ snap })` — `snap` is the one presentational decision there is to lock, because which values a scale admits at all is a property of the consumer's world. Curried variant: **`ContinuousMarkedSlider`** — no grid, so the thumb tracks the pointer to the finest unit the domain can express. ONE variant deliberately: every other grid is a consumer's own unit, curried once with the factory.
  - **`geometry.ts` is private and prints as a table.** Every number the slider paints is decided there, it is pure, and `geometry.test.ts` reads a whole row of marks headless — headless observation before the GUI. It also pins the stylesheet: the CSS vars for the track's inset, the canvas size and the shrink floor are asserted against the constants the drawing uses, so a change to one fails until the other follows.
  - Example:
    ```tsx
    import { ContinuousMarkedSlider } from "@primestageprime/solid-ui-components";
    ...
    <ContinuousMarkedSlider
      ...
      deltaLabel={value() === 44 ? null : `+${value() - 44}`}
      onChange={setValue}
    />
    ```

## MutationSliders
- **MutationSliders** — Composite (Depth 3). A row of VERTICAL dials, one per named entity, answering three questions at once: what is this entity allowed, where was it, and where is it going? The model is **entirely generic** — an `entity` has an `id`, a `label`, a prior amount (`old`), a future amount (`value`) and an allowed `range`, all in the CONSUMER'S OWN UNITS — and the component runs no arithmetic beyond clamping and formats nothing itself. The track runs the whole shared `domain`, so every dial in the row is on ONE scale and two entities are comparable at a glance. On it: the shaded **allowed range** (the entity's `range`, which is NOT the size of the change — two entities with the same allowance draw the same box however far each moved), a muted **prior** arrowhead, an accent **future** arrowhead pointing at the track from the opposite side so a pair at the same amount meets nose to nose, a wider **change line** toned `raise`/`cut` between them, and the **signed delta** as a figure level with that line's midpoint (hue is never the only cue — future-above-prior says the same thing by position). Under the dial, the future amount through the caller's `format`, with `was <prior>` muted beneath it and nothing at all when the amount did not move. **Three presences, three shapes.** Present is both amounts. **Removed** is `value: null` — not a fall to the bottom of the range: the name strikes through, the range and prior arrow stay, the future arrow goes, and the readout is `—`. **New** is `old: null`: the range and the future arrow, nothing coloured, and the readout is `labels.new`. **THE RANGE IS THE CLAMP** — both amounts are pulled onto it before they are drawn, the thumb announces the clamped figure, and `onChange` never emits outside it, so the thumb stops dead at an edge. Key props: `entities` (`readonly MutationEntity[]`, drawn in the order given — that order is the reading order), `onChange` (`(id, value) => void`, fired on every intermediate unit of a drag), `onChangeEnd?` (fired ONCE per committed gesture — take this one for persistence or anything that recomputes), `domain?` (`[min, max]`; **omitted it is DERIVED** from the entities, lowest range floor to highest ceiling, so the ranges fill the dial's full height — pass one only to hold the track STILL), `snap?` (round every emitted amount onto a grid; the range still wins at an edge that is not a multiple of it, and it raises the arrow-key step when coarser), `labels?` (`MutationSliderLabels` — see below), `onRemove?` / `onRestore?` (the ⊗ and ↺ that SHARE one footer slot; omit either and that affordance is not drawn), `onAdd?` (the `+` at the end of the row, which stays visible past the last page on purpose), `selected?` / `onSelectionChange?` (the pinned multi-selection — controlled when `selected` is supplied, otherwise the component holds it), `format?` (`(value: number) => string`, default `String`). Exported types: `MutationSlidersProps`, `MutationSlidersOverrides`, `MutationSlidersDataProps`, `MutationSliderLabels`, `Entity`, `Domain`, `ChangeTone` — plus the qualified aliases `MutationEntity` and `MutationSlidersDomain` for a call site far from this component. **Owns NO CSS and no intrinsic elements** (2026-09-17, Peter's axiom: nothing above Depth 1 contains anything but existing SUI components). The dial is the `MarkedSlider` Primitive, which owns the stylesheet this folder used to carry; the row's fill chain is `FillStretchRow`; the name toggle is `PressableLabelButton` + `NowrapLabel`; the two readouts are `SteadyMonoValue` and `SteadyMonoMeta` (with `ReservedMonoMeta` for the state that has nothing to report and still has to hold its space); the one footer slot is `GlyphSlotGhostButton`, or `ReservedGlyphSlotButton` where the consumer supplied no action. `rows.ts` holds what is left and is pure: how many dials fit, which window shows, and what a move means when more than one is pinned. Uses `--sui-success`, `--sui-danger`, `--sui-accent`, `--sui-text-secondary`, `--sui-text-muted`, `--sui-border`, `--sui-font-mono`. Use for: a before/after scenario editor where several bounded quantities move together and the reader needs to compare them — a budget revision, a capacity plan, a pay review. For ONE bounded value, use `Slider`; for one value against a reference, use `RateGauge`; for a value axis carrying named thresholds, use `BandRail`.
  - **Nothing in it knows what its entities ARE, and `labels` is why.** The three words the dial says on a consumer's behalf are props: `labels.remove` (the footer verb on a present entity, used as the button's accessible name with the entity's own label after it — `Remove Ana`), `labels.restore` (the verb on a removed one) and `labels.new` (the readout where `was <prior>` would go). They default to `"Remove"` / `"Restore"` / `"New"`, and a partial object fills only the gaps it leaves. A consumer whose entities are people passes `{ remove: "Terminate", restore: "Reinstate", new: "new hire" }`; one whose entities are line items passes `{ remove: "Cancel", new: "added" }`. There is no `"pay"`, `"salary"`, `"hire"` or currency anywhere in the component.
  - **Selecting two names PINS them.** Clicking a name toggles it (a real `<button>` with `aria-pressed`, so the state is never colour-only). The moment a selection reaches two, every member snaps to the HIGHEST amount among them — levelling one DOWN by accident is the expensive mistake — and thereafter dragging any one of them moves all of them by the same delta, each clamped to its OWN range, so one hitting its ceiling stops there while the rest carry on. A single selected dial still drags alone.
  - **It measures itself, in both directions.** The WIDTH drives paging: when the dials do not all fit the row grows a chevron at each end, shows as many WHOLE dials as fit and **never fewer than one**, and pages by ONE dial rather than by a screenful — the row exists to be compared across, and a full-page jump means no two neighbours either side of a boundary are ever on screen together. The window announces itself (`role="group"`, `aria-label="dials 3–5 of 7"`). The HEIGHT lengthens the track through `height: 100%`, which computes to `auto` against a parent of indefinite height, so one declaration serves a sized card and a content-sized column alike with no `fill` prop. Below `MIN_DIAL_HEIGHT` (180px) the dial stops shrinking and overflows, which is visible and fixable, rather than becoming silently illegible.
  - **A drag and a key press move by different steps, deliberately.** Kobalte's `step` governs the pointer as well as the keyboard, so it gets the FINE one (the smallest unit the domain can express) and the arrow keys are intercepted in the capture phase and moved by a derived "nice" step instead — a hundredth of the span rounded up to 1, 2 or 5 × a power of ten. Shift+arrow and PageUp/PageDown move ten of those; Home and End run to the domain ends and are clamped onto the range. There is no `step` prop: a step is a property of the scale, and the scale is already here.
  - **The geometry is private and prints as a table.** The marks live in `MarkedSlider/geometry.ts` beside the Primitive that paints them and the row math in `rows.ts`; every number either one decides, it is pure, and `geometry.test.ts` reads a whole row headless — headless observation before the GUI. Its ~30 FUNCTIONS are deliberately NOT published; the three TYPES are, because a consumer has to be able to name the shape of the `entities` it builds — `Entity`, `Domain` and `ChangeTone` are the canonical names and reach the root barrel unambiguously (verified: no other component's barrel re-exports any of the three, and RateGauge publishes its own as `RateGaugeDomain` for exactly this reason). An ambiguous `export *` resolves to nothing at all, so if a second component ever wants one of these words, that one qualifies. One entity's column is a real seam (`dial.tsx`, `MutationDial`) but is private for now: nobody has asked for a standalone dial, and a published component owes a showcase and a catalog entry.
  - Factory: `createMutationSliders({ format, labels, snap })` — the unit, the vocabulary and the grid are all properties of the CONSUMER'S world rather than of a given row, which is exactly the set a variant should carry (`format` is an override here for the same reason it is on `Slider` and `BandRail`). Curried variant: **`NumberMutationSliders`** — plain counted numbers grouped through the reader's locale, the neutral verbs, and no grid. ONE variant deliberately: any other combination is a consumer's own domain, and SUI should not guess which currency or which verb.
  - **Unreleased (payroll board, 2026-09-24), additive:** `readout: "stacked" | "beside"` joins the overrides (type `MutationSlidersReadout`; default `"stacked"` is the layout above, unchanged). `"beside"` draws the OLD amount beside the prior arrowhead (MarkedSlider's new optional `priorLabel`), the new amount under the dial, and the difference RIGHT of the bar on two lines, `+$5K` over `(4%)` (a `deltaLabel` containing a newline draws as lines). Both side labels run into the gap between dials, so a `"beside"` row uses **`GutteredFillStretchRow`** (new Layout variant: `FillStretchRow` at the new `xl` 24px row gap) and pages by the matching slot. Second curried variant **`CompactCurrencyMutationSliders`** locks compact dollars (`$125K`, "$" hardcoded until i18n), `readout: "beside"`, `snap: 1_000`, `grouping: "link"`, `precision: -3` and `labels: { remove: "Delete" }` (the footer says `Delete Ana`). Override `precision?: number` makes the amount under each dial editable in place (a `SliderField`: Enter/blur commit, Esc reverts); a typed figure is rounded to that power of ten (`2` → 0.01, `-3` → thousands), then clamped to the entity's range, and moves the dial like a key press (a linked group follows). New override `grouping: "pin" | "link"` (default `"pin"`, unchanged). `"link"` adds a link button under each name (shown on hover/focus, always on a selected dial) that toggles the SAME `selected` / `onSelectionChange` selection the name click does — one group, two triggers, `active` ring as its mark — and moves the selection to the same AMOUNT rather than by the same delta, so a group a ceiling split re-levels on the next move (`CompactCurrencyMutationSliders` locks it). The consumer persists `selected`. ITEMS: an entity's optional `item?: string` groups CONSECUTIVE dials of one item (a person with two positions); the row then pages by whole items (never splitting one) and draws each multi-dial run inside the `itemFrame` override (a consumer-curried frame component; unset, no background). The Surface variant **`ItemTintSurface`** (borderless, padding-free, faint accent wash) is that frame, and `CompactCurrencyMutationSliders` locks it. `onReset` — supplied with `onRemove`, each present dial's footer becomes a split Reset | Delete pair; the consumer owns any confirm on Delete.
  - Example:
    ```tsx
    import {
    ...
    // Or curry the consumer's own unit, vocabulary and grid once.
    const BudgetSliders = createMutationSliders({
...
- **PairedMutationSliders** — Composite (Depth 3). A row of named entities, each with **TWO differently-united dials under one name** — hours a week and dollars an hour, headcount and cost, quantity and price. A **sibling of `MutationSliders`, not a generalisation of it**: that component documents its `domain` as "the shared `[min, max]` the TRACK runs… Every dial in the row uses it, which is what makes two dials comparable", and two measures in different units share no domain, no formatter and no snap grid — so the prop that is its whole reason for existing is the prop that disqualifies this shape. Widening it to N measures would re-shape `onChange`, `format`, `snap`, `selected` and the pinning maths for its two live consumers: a breaking change where a sibling is additive. Nothing in `MutationSliders` is deprecated by this. The model is **entirely generic** — an entity has an `id`, a `label` and `measures: [PairedMeasure, PairedMeasure]`, each measure a `prior`, a `value` and an allowed `range` in ITS OWN units — and the component runs no arithmetic across the two measures and could not: what a pair means TOGETHER is knowledge only the consumer has, which is what `summary` is for. Each dial is the `MarkedSlider` Primitive and draws exactly what it draws on `MutationSliders`: the shaded allowed **range**, a muted **prior** arrowhead, an accent **future** arrowhead, a **change line** toned `raise`/`cut` between them and the **signed delta** level with its midpoint. Under each dial, that measure's amount through THAT axis's `format`, with the axis's name (`Hrs/wk`, `$/hr`) muted beneath it — or `labels.new` where the measure has no prior amount. **THE RANGE IS THE CLAMP**, per measure. Key props: `entities` (`readonly PairedMutationEntity[]`, drawn in the order given), `axes` (`[PairedMeasureAxis, PairedMeasureAxis]` — see below; supplied ONCE for the whole row, position for position with each entity's `measures`), `onChange` (**`(id, measureIndex, value) => void`** — the index is the difference between this component and a row of single dials; a consumer that ignored it would write an hours figure into a rate), `onChangeEnd?` (same signature, fired ONCE per committed gesture), `summary?` (`(entity) => string`, the consumer's own one-line reading of the pair printed under both dials — omitted, the line is not drawn at all; return `""` for an entity with nothing to say and the space is kept), `labels?` (`PairedMutationSliderLabels`), `onRemove?` / `onRestore?` (the ⊗ and ↺ that SHARE one footer slot), `onAdd?` (the `+`, which stays visible past the last page on purpose), `selected?` / `onSelectionChange?` (the pinned multi-selection). Exported types: `PairedMutationSlidersProps`, `PairedMutationSlidersOverrides`, `PairedMutationSlidersDataProps`, `PairedMutationSliderLabels`, `PairedMeasureAxis`, `PairedMeasureAxes`, `PairedMeasure`, `PairedMutationEntity`, `MeasureIndex` — **every one qualified on purpose**: `Entity`, `Domain` and `ChangeTone` belong to `MarkedSlider`/`MutationSliders` and have live consumers, and an ambiguous `export *` resolves to NOTHING silently, so `barrel.test.ts` pins both sets through the package root. **Owns NO CSS and no intrinsic elements** (Peter's axiom: nothing above Depth 1 contains anything but existing SUI components). Uses `--sui-success`, `--sui-danger`, `--sui-accent`, `--sui-text-secondary`, `--sui-text-muted`, `--sui-border`, `--sui-font-mono` — all of them through `MarkedSlider`, the Text variants and the Layout variants it composes. Use for: a before/after editor where each named thing has TWO bounded quantities that move independently — an hourly board (hours × rate), a purchasing plan (quantity × unit price), a staffing plan (heads × cost). For ONE quantity per entity, use `MutationSliders`; for MORE than two, or for measures that read as captioned groups, use `GroupedMutationSliders` — the N-measure sibling this component is the N=2 case of; for one bounded value, use `Slider`; for one value against a reference, use `RateGauge`.
  - **An AXIS is everything true of a measure rather than of an entity**, and it is supplied once at the component because "the second dial is dollars an hour" is a fact about the row — repeating it per entity would be a fact that can disagree with itself. A `PairedMeasureAxis` carries `label?` (the muted name under the dial — the only thing telling a reader which of the two dials is which), `format?` (`(value) => string`, default `String` — hours and dollars do not share a formatter any more than they share a scale), `snap?` (the grid this measure's emitted values land on, which also raises the arrow-key step when it is coarser) and `domain?` (**omitted it is DERIVED** from the entities' ranges AT THAT POSITION, lowest floor to highest ceiling — pass one only to hold that track still). Every field is optional, so `[{}, {}]` draws.
  - **Pinning is MEASURE-LOCAL, and that is structural rather than conditional.** Clicking a name toggles it; the moment a selection reaches two, each measure levels against its OWN peers — the hours against the hours and the rates against the rates — and thereafter dragging one dial moves the SAME measure of every selected peer by the same delta, each clamped to its own range. There is no branch anywhere enforcing that: the row PROJECTS its paired entities down to single-measure `Entity` views, one projection per index (`pairs.ts`), and hands them to the pin arithmetic `MutationSliders/rows.ts` already owns. Measure 0 and measure 1 are two disjoint projections, so a pin on one cannot reach the other.
  - **REMOVED is both measures at `null`**, and there is no flag. `MutationSliders` says removal with `value: null` and nothing else; a `removed` boolean beside two nullable values would be a second copy of the same truth that can disagree with itself. An entity with ONE measure still holding a value is not removed — it is an entity whose other measure has nothing to say yet, which is a real state and a different drawing. `onRemove` is the consumer's cue to null both.
  - **It measures itself, and it pages by whole PAIRS.** A page that split a pair would put an entity's first measure on screen and its second off it, which is precisely the comparison the pair exists to make. `MutationSliders/rows.ts`'s `rowLayout` therefore takes the slot width as a trailing, optional argument (additive, 2026-09-17; every existing caller keeps `DIAL_SLOT` and its tests are unchanged), and this row passes `PAIR_SLOT` — exactly `2 * DIAL_SLOT`, because the pair sits in the same `sm`-gap (8px) row the outer row uses, so there is no third constant to drift out of step. Minimum ONE whole pair, chevrons at each end when it pages, `role="group"` announcing the window. The HEIGHT lengthens both tracks.
  - **Factory only — `createPairedMutationSliders({ axes, labels })` — and no curried variant, deliberately.** "Clients import only curried components" is satisfied by the FACTORY: a variant here would have to name two units, two names, two grids and two scales, and every combination anybody has asked for so far is DOMAIN-SPECIFIC (`"Hrs/wk"` 0–80 snap 1 × `"$/hr"` 0–300 snap 5 is an hourly board's axes, not a library's). SUI does not guess which units (SUI: start minimal, expand on demand). The consumer curries once and imports its own curried name thereafter, exactly as the showcase does. The first non-domain-specific pair anybody demands gets a variant here.
  - Example:
    ```tsx
    import {
    ...
    // Curry the consumer's own two units, names, grids and verbs once.
    ...
    <HourlySliders
      entities={services()}
      summary={(e) => weeklyBilling(e)}   // the consumer's own arithmetic
      onChange={(id, measure, value) => setMeasure(id, measure, value)}
      onRemove={(id) => dropBothMeasures(id)}
      onAdd={append}
    />
    ```

## GroupedMutationSliders
- **GroupedMutationSliders** — Composite (Depth 3). A row of named entities, each with **N differently-united dials under one name, in CAPTIONED GROUPS** — a monthly seat count and its fee beside an annual count and its share, a count and a rate beside a service level. The **N-measure sibling of `PairedMutationSliders`**, which is the N=2 case, and a sibling rather than a generalisation of either neighbour for the reason the whole family is siblings: `MutationSliders` documents its `domain` as the shared `[min, max]` every dial in the row runs on, and measures in different units share no domain; `PairedMutationSliders` bakes the pair into its **types** (`measures: readonly [PairedMeasure, PairedMeasure]`, `MeasureIndex = 0 | 1`), so four dials under one name is not a prop away — it is unexpressible. Re-expressing either over this one would re-shape public types for live consumers, where a sibling is additive (add / deprecate / delete). **Nothing in `MutationSliders` or `PairedMutationSliders` is deprecated by this.** The model is **entirely generic** — an entity has an `id`, a `label` and `measures: readonly GroupedMeasure[]`, each measure a `prior`, a `value` and an allowed `range` in ITS OWN units — and the component runs no arithmetic across the measures and could not: what they mean TOGETHER is knowledge only the consumer has, which is what `summary` is for. Each dial is the `MarkedSlider` Primitive and draws exactly what it draws on its two siblings: the shaded allowed **range**, a muted **prior** arrowhead, an accent **future** arrowhead, a **change line** toned `raise`/`cut` between them and the **signed delta** level with its midpoint. Under each dial, that measure's amount through THAT axis's `format`, with the axis's name muted beneath it — or `labels.new` where the measure has no prior amount. **THE RANGE IS THE CLAMP**, per measure. Key props: `entities` (`readonly GroupedMutationEntity[]`, drawn in the order given — that order is the reading order), `axes` (`GroupedMeasureAxes` — supplied ONCE for the whole row, position for position with each entity's `measures`; **its length is the row's measure count**), `onChange` (**`(id, measure, value) => void`**, the index saying WHICH measure — a consumer that ignored it would write a seat count into a discount), `onChangeEnd?` (same signature, fired ONCE per committed gesture — take this one for anything expensive), `summary?` (`(entity) => string`, the consumer's one-line reading of a whole entity printed under every dial; omitted, the line is not drawn at all — return `""` for an entity with nothing to say and the space is kept), `labels?` (`GroupedMutationSliderLabels`, defaulting to `Remove` / `Restore` / `New`), `onRemove?` / `onRestore?` (the ⊗ and ↺ sharing one footer slot), `onAdd?` (the `+`, which stays visible past the last page on purpose), `selected?` / `onSelectionChange?` (the pinned multi-selection, controlled when supplied). Exported types: `GroupedMutationSlidersProps`, `GroupedMutationSlidersOverrides`, `GroupedMutationSlidersDataProps`, `GroupedMutationSliderLabels`, `GroupedMeasureAxis`, `GroupedMeasureAxes`, `GroupedMeasure`, `GroupedMeasureIndex`, `GroupedMutationEntity`, `GroupRun` — **every one qualified on purpose**: `Entity`, `Domain`, `ChangeTone` and `MeasureIndex` belong to `MarkedSlider` / `MutationSliders` / `PairedMutationSliders` and have live consumers, and an ambiguous `export *` resolves to NOTHING silently, so `barrel.test.ts` pins both sets through the package root. **Owns NO CSS and no intrinsic elements** (Peter's axiom: nothing above Depth 1 contains anything but existing SUI components). Uses `--sui-success`, `--sui-danger`, `--sui-accent`, `--sui-text-secondary`, `--sui-text-muted`, `--sui-border`, `--sui-font-mono` — all of them through `MarkedSlider`, the Text variants and the Layout variants it composes. Use for: a before/after editor where each named thing carries THREE OR MORE bounded quantities that move independently and read in captioned families — a pricing board (monthly count × fee, annual count × share), a capacity plan (heads × cost × utilisation). For exactly two, use `PairedMutationSliders`; for one quantity per entity, use `MutationSliders`; for one bounded value, use `Slider`. `entityFrame?` (Override): a component drawn around each entity's column (name + dials) — curry `ItemTintSurface` for a light background per entity; borderless/padding-free, so paging is unchanged. Omitted, nothing changes. **Consumers set `entityFrame` in their own curry** (e.g. thorcasting's `LicenseSliders`): SUI ships no curried variant with it, because a variant would have to lock `axes`, which fixes each consumer's dial count.
  - **The GROUP is the one idea it adds over a paired row.** `axes[i].group` names a caption, and **CONSECUTIVE** axes sharing a name are drawn under one — so four dials read as two pairs rather than four strangers. Consecutive and not "every axis with this name", because the alternative silently reorders a row the consumer ordered on purpose: axes named `mo, yr, mo` draw **three** runs. An axis naming no group stands in a run of its own with the caption line **RESERVED** rather than absent, so every dial in the row still sits at one height; a row where NO axis names a group pays no caption line at all.
  - **The measure index is a NUMBER, not a union**, and that is the type-level whole of the difference from `PairedMutationSliders`. The count is the consumer's, so there is no finite set of positions to name, and `GroupedMeasureIndex = number`.
  - **Pinning is MEASURE-LOCAL, and it is structural rather than conditional.** Clicking a name toggles it; the moment a selection reaches two, each measure levels against its OWN peers, and thereafter dragging one dial moves the SAME measure of every selected peer by the same delta, each clamped to its own range. There is no branch enforcing that: the row **projects** its entities down to single-measure views, one projection per index, and hands each to the existing row arithmetic — measure *i* and measure *j* are disjoint projections, so a pin on one cannot reach the other.
  - **REMOVED is EVERY measure at `null`**, and there is no flag. A `removed` boolean beside N nullable values would be a second copy of the same truth that can disagree with itself. An entity with one measure still holding a value is not removed.
  - **It measures itself in both directions, and it pages by whole ENTITIES.** The WIDTH drives paging and the HEIGHT lengthens every track. A page that split an entity would put its first group on screen and its second off it, which is precisely the comparison the grouping exists to make — so the row's slot is `axes.length` dials wide, and a container measured too narrow still shows one whole entity. **A grouped column needs ≈325px of height**; a box bounded below that hands the row less than the slider's own 180px floor and the summary prints on top of the readouts, so that is the figure a consumer budgets for. One operational note: the size observer defers through `requestAnimationFrame`, which a browser SUSPENDS for a hidden document — check `document.visibilityState` first from an automation harness, or a working component is indistinguishable from a broken one.
  - **Factory only — `createGroupedMutationSliders({ axes, labels })` — and no curried variant, deliberately.** The same call `PairedMutationSliders` makes, one notch stronger. "Clients import only curried components" is satisfied by the FACTORY, and here a variant could not merely guess wrong about presentation: **`axes` is required, and `axes.length` IS each entity's measure count**, so a curried name would dictate how many dials every consumer's entities carry. There is no generic N the way there is a generic number for `NumberMutationSliders`. Every combination anybody has asked for is domain-specific (a licence board's monthly `#`/`$` and annual `#`/`%`), and SUI does not guess which units (SUI: start minimal, expand on demand). The consumer curries once and imports its own curried name thereafter, exactly as the showcase does.
  - Example:
    ```tsx
    import {
    ...
    // Curry the consumer's own axes, captions and verbs once.
    ...
    <LicenseSliders
      entities={products()}
      summary={(e) => monthlyEquivalent(e)}   // the consumer's own arithmetic
      onChange={(id, measure, value) => setMeasure(id, measure, value)}
      onRemove={(id) => nullEveryMeasure(id)}
      onAdd={append}
    />
    ```
...
- **MutationToolbar** — Composite (Depth 2). The header row of a "changes" panel: a title, the as-of chips that pick which change is being edited (a `SegmentedControl`), and the panel's actions. Composes `SpreadRow` / `ClusterRow`, `TextTitle` / `NoteText`, `SegmentedControl`, `GhostButton` / `PrimaryButton` / `DangerButton` and `Icon`; owns no CSS. **With no changes the chips' slot holds `emptyNote`** — never an empty bar, which would be a control that cannot be operated. **An action renders only when its callback is passed** (`onAdd`, `onReset`, `onSave`), so a screen with no Save draws none. **A change is removed from its own chip**: `onRemove?(id)` puts an × on each chip (hover or focus reveals it, Delete on the focused chip fires it, a touch screen always shows it) — the thing being removed is the thing under the pointer, which a corner button could never say. **Reset is the `undo` glyph in an `IconOnlyButton` inside a `Tooltip`**, not a word: `labels.reset` becomes both the tooltip and the accessible name, so a curried vocabulary still owns the word. Key props: `title`, `changes` (`readonly MutationToolbarChange[]` — `{ id, label, removable? }`, in chip order; `removable: false` opts one chip out of `onRemove` — no × at all, not a disabled one, and Delete does nothing — e.g. a hire flag the app can't delete; the chip is constant-width either way, since a removable chip always reserves its fading ×), `selected` (`string | null`), `onSelect(id)`, `emptyNote`, `onAdd?`, `onReset?`, `onSave?`, `saveDisabled?`, `onRemove?(id)`, `onDelete?(id)` (**deprecated** — the corner Delete that `onRemove` supersedes; still honoured, and still renders only while a change is selected). The words are an Override: `createMutationToolbar({ labels: { add, reset, save, delete, chips } })` curries a screen's own vocabulary ("Hire" rather than "Add"); defaults are `DEFAULT_MUTATION_TOOLBAR_LABELS`. No shipped curried variant — the default words are already the neutral ones. Use for: the header of any panel that edits a list of dated changes — the Hourly and License Board benches, and a consumer's payroll or hourly builder.
...
## ChannelChart
- **ChannelChart** — Composite (Depth 2). A band (the CHANNEL) with a running value inside it, on an index period axis (`xDomain = [-0.5, n-0.5]`). Curried variants: **`ChannelChart`** (the channel plot alone) and **`ChannelDivergenceChart`** (the channel plot above a diverging-bar plot that shares its x-scale AND its margin, so every bar sits under its point). Composes `Chart` ×2 + `Grid` + `XAxis` / `YAxis` + `AreaSeries` (with `lower`) + `LineSeries` + `PointSeries` + `BarSeries` + `ReferenceLine` + `ChartLabels` + `ChartTooltip`, `Legend`, `MutedEmptyState`, inside `FillColumn` / `GrowFillBox`; owns no CSS — tones are SUI token strings passed as props (`DEFAULT_CHANNEL_CHART_TONES`: band `--sui-accent`, line `--sui-text-primary`, over `--sui-danger`, under `--sui-warning`, zero `--sui-text-muted`). **It fills its box** (the `StackedTimelineChart` measurement: `getBoundingClientRect` in `onMount`, then `observeSize`; width and height taken independently, a zero reading never stored, `CHANNEL_CHART_FALLBACK_SIZE` 520×360 until measured), so give it a sized parent. Data props (`ChannelChartDataProps`): `periods` (`readonly ChannelPeriod[]` — `{ key, label?, lo, hi, value, outside?, diff? }`: `lo`/`hi` the band at that period, `value` the running value; `outside` overrides the marker tone, which is otherwise read off `value` against `lo`/`hi`; `diff` is the period's signed distance OUTSIDE its own per-period band, drawn by the divergence plot only — the caller's, never derived, because bars show per-period distance while markers show cumulative position), `formatValue` (every printed number — y ticks, end labels, bar labels, tooltip — so money formatting stays the caller's), `ariaLabel` (the whole chart is one `role="img"`), `selectedKey?` (a solid rule on both plots), `onPickPeriod?` (a click on either plot, snapped to the nearest period's `key`), `emptyMessage?`, `legend?` (`ChannelChartLegend` text; omitted, no legend). Overrides (`ChannelChartOverrides`, curried with `createChannelChart`): `showDivergence`, `divergenceShare` (height share of the bar plot, default 0.45), `labelBars`, `labelEnds` (last period's value / ceiling / floor in the right gutter), `markerRadius` (the last marker draws 1.4× larger), `tones` (`ChannelChartTones`). Rules: y-domain `[min(0, lo, value), max(hi, value)]` with 5% headroom, so a refund that turns a period negative stays on screen; the divergence domain is symmetric around zero; a zero bar draws as a flat 3px tick (`BarSeries` drops zero segments, so it is a `+half`/`-half` pair); an all-zero or empty input widens its domain instead of producing NaN. **Headless first:** everything drawn comes from the pure `channelModel(periods, { showDivergence, labelEnds, formatValue })` in `ChannelChart/channelGeometry.ts` — domains, explicit ticks, the ONE margin (left from the widest tick label across BOTH plots, right from the widest end label), markers, bars, end labels — and `formatChannelTable(model)` prints `i | key | lo | hi | value | outside | diff | tone`; `channelGeometry.test.ts` prints the STAX fixture. Both functions are INTERNAL (not exported), as `buildStackedArea` is — a consumer composes the chart, not the geometry; the types a caller states its data with (`ChannelPeriod`, and `ChannelModel`, `ChannelRow`, `ChannelMarker`, `ChannelBar`, `ChannelEndLabel` with their tone types) are exported. Not `StackedTimelineChart`, which is a stacked composition from a floor. Not `CashflowScrubChart`, which is a day ribbon plus a running balance. Not `DeviationBand`, which is a sign-coloured difference between two lines. Use for: projection vs actual reconciliation, a budget envelope, a forecast cone with actuals.
  - Example:
    ```tsx
    import { ChannelDivergenceChart } from "solid-ui-components";

    <FixedHeightBox>
      <ChannelDivergenceChart
        periods={channelPeriods(coverage)}
        formatValue={dollarsFromCents}
        ariaLabel="STAX cumulative channel, 2026-01 to 2026-09"
        selectedKey={month()}
        onPickPeriod={setMonth}
      />
    </FixedHeightBox>
    ```
...
- **StackedTimelineChart** — Composite (Depth 2). Stacked bands over time that **fill their box**, with an optional captioned horizontal rule, one numbered vertical rule per event, a hover readout and a pick. Composes `Chart` + `Grid` + `XAxis` / `YAxis` + `StackedAreaSeries` + `ReferenceLine` + `ChartTooltip` inside a `GrowFillBox`; owns no CSS. **It measures its own box** — `Chart` takes pixels and has no fill — the documented way: first size synchronously from `getBoundingClientRect` in `onMount`, then `observeSize`; a zero reading is never stored, so an unmeasured chart draws at `STACKED_TIMELINE_FALLBACK_SIZE` (640×220) rather than blank. **The pick is NOT snapped**: `onPick(at: Date)` reports the raw date and the caller snaps to its own grid, the `Chart.onPick` contract. Key props: `series` (`readonly StackedAreaSeriesData[]`, bottom first, step-valued), `xDomain` (`[Date, Date]`), `yDomain` (fixed, so bands stay comparable across edits), `xTickValues?`, `rule?` (`StackedTimelineRule` — `{ value, label }`, dashed), `events?` (`readonly StackedTimelineEvent[]` — `{ at, label }`; a `Mutation[]` fits as-is), `hoverLabel?` (`(at: number) => string`, tracks the pointer), `onPick?`. Overrides, curried with `createStackedTimelineChart({ curve, margin, xTickFormat, yTickFormat })` — tick text carries a screen's units, so there is no shipped variant. **`curve` decides how the stack crosses a change**, forwarded to `StackedAreaSeries`: `"smoothStep"` (the default) spends x on the crossing, so a band rises before a change and falls after it — the picture of a quantity that varies continuously. `"linear"` spends none: a value holds flat across its own step and the change lands square on its own x. **Curry `"linear"` when the x-axis carries BUCKETS rather than a continuum** — a month's cash, a week's hours. A bucket has no interior, so the smoothed shoulders draw a figure the model never produced, and a one-bucket spike reads as a rise and a fall instead of as one payment. It curries with the tick text because whether an axis carries a continuum or a row of buckets is one fact about a screen's data, not a per-render choice. **`columns` switches the MARK from bands to COLUMNS**, and it is the better answer where `curve: "linear"` is only a partial one. Pass the bucket STARTS (`readonly TimeValue[]`; the last bucket runs to `xDomain[1]`, so 24 month starts draw 24 columns) and the stack renders through `BarSeries` instead of `StackedAreaSeries`, one column per bucket per band, holding what each series carries at that bucket's start. `curve` no longer applies. **The caller owns the edges** because a bucket is a fact of ITS calendar and this chart knows no calendar — the same reason `onPick` reports a raw date; deriving buckets from the series' own points would give uneven columns, because a point marks a CHANGE and not a period. `columnWidth?` (default `0.84`, curried) is the column's share of its bucket, so the remainder is the GUTTER that makes the buckets read as separate. Unequal buckets (a 28-day month beside a 31-day one) draw at their OWN widths, so February's column is narrower and every GAP comes out the same — one mean width instead put all of the 28-to-31-day difference into the gaps (4.7 px beside 6.5 px) and pushed an event rule off the gap's centre. `segmentGap?` (curried, default `1`) leaves that many pixels of GROUND between a column's segments: the series palette sits in a narrow lightness band, so two neighbouring bands are near EQUILUMINANT, the boundary between them carries hue but almost no luminance, and the eye — which finds edges by luminance — stops reading the two segments as the same width. It is a gap and not a stroke because a stroke straddles a rect's edge by half a pixel, and `BarSeries` snaps every edge to a whole pixel on purpose. `selectedEvent?` (an index into `events`) draws THAT rule solid, 2px, accented and full-strength, with its number accented too; the rest stay dashed. Solid-versus-dashed carries the state alone, so the accent is never the only cue, and an out-of-range index selects nothing — a caller's `findIndex` miss reads correctly with no special case. The index is the identity because the rules render through `Index`: callers rebuild the list wholesale on every edit. **With `columns` AND `onPick`, the chart GHOSTS the rule a click would leave** on the hovered bucket's start: a `ReferenceLine` at `opacity` 0.24, so the ghost IS the rule it becomes. It reads the same `hoverX` mapping `Chart.onPick` reports from, so the two cannot drift. It needs the grid and draws nothing without it — `Chart.onPick` is deliberately unsnapped, so a ghost on the raw x would promise a spot the caller's own snapping then moves, which is worse than no ghost. `GhostPin` is the same idea for a glyph. The pure half is `stackBuckets(series, edges)` in `stackedArea.ts`, which reads the same `valueAt` the band mark reads, so a column and a band can never disagree about what a series holds. Use for: hours or capacity by source over a year with a threshold and dated changes — the Hourly Board's Work Mix; with `columns`, a month's cash by source — the License Board's mix. For levels that are rails rather than bands, use `LevelsTimeline`. Size-responsive chrome (G17), from its measured box: under 200px tall, 3 y ticks (domain ends + middle) and a tighter default inset; under 400px wide, every other caller x tick. A curried `margin` wins. Pure rule: `stackedTimelineLayout` in `layout.ts`. No props.
...
- **createAxisWaterMarks** — hook (`src/hooks`). A held y-DOMAIN: the ceiling AND the floor **expand with the data at once and shrink only on `reset()`**, so an edit that lowers a value moves the data, never the axis under the pointer, and one shrink control fits it back. Built on `createHighWaterMark` twice — the floor is the ceiling ratchet on the negated minimum — so the snap-up / ease-down motion is that hook's. `createAxisWaterMarks(fitted, { transitionMs? })` takes an accessor for the current `FitDomain` (`{ min, max }`, or `null` when nothing is drawn) and returns `AxisWaterMarks` `{ domain, reset }`; `domain()` is `[min, max]`, or `null` until a fit has been seen. The pure step is **`holdFitDomain(previous, epoch, fitted)`** over a `HeldDomain` (start from `NO_HELD_DOMAIN`, read with `heldDomainOf`); **`createHeldFit()`** returns `HeldFit` `{ hold, reset }` for a fit computed inside another memo, where no signal may be written; **`observeAxisWaterMarks(frames)`** is the headless observation — `WaterMarkFrame`s in (a fit, optionally after a reset), a table of `WaterMarkRow`s (fitted vs held) out. Unit-free. Promoted from thorcasting's `~/lib/axisWaterMarks` (2026-09-24), which can now delete its copy. For `LevelsTimeline`, the fit is `levelsValueFit(levels)`. Use for: any chart whose y-range the reader edits live — pay levels, cash flow, hours.

    ```tsx
    const payAxis = createAxisWaterMarks(() => levelsValueFit(levels()));
    <IconOnlyButton onClick={payAxis.reset} aria-label="Fit y-axis to current values"><Icon name="shrink" size="sm" /></IconOnlyButton>
    <PayLevelsTimeline levels={levels()} valueDomain={payAxis.domain() ?? undefined} … />
    ```
...
- **Slider** — Atomic (Depth 1). A labelled range control that prints its own live value. Owns `Slider.css`; wraps `@kobalte/core/slider` (matches the Combobox/Select/Tooltip/Toast/ThemedNumberInput/DateRangePicker Kobalte-wrapping pattern — see CONTEXT.md). The label line carries the caption on the left and `format(value)` right-aligned on the right, so a control reads `Safety buffer` / `6 months` on one line and needs no separate readout beside it. **The value stays in the CONSUMER'S OWN UNITS** — the component runs no arithmetic beyond Kobalte's step snapping and formats nothing itself, so a dial that keeps integer cents passes cents and supplies a `format` that renders dollars. Assuming dollars would put a unit in the widget that only the consumer knows. Key props: `value` (`number`, controlled), `onChange` (`(value: number) => void`), `min`, `max`, `step` (default `1`), `label` (accessible name and visible caption — required), `format?` (`(value: number) => string`, default `String`), `disabled?`, `editable?` (`boolean`, default off — turns the readout into a field; see the note below), `valueLabel?` (`JSX.Element` — a node drawn in place of the readout; forbidden beside `editable`, see the note below), `ticks?` (`boolean | readonly number[]` — notches drawn on the track: `true` marks every `step` from `min` to `max` inclusive, an array marks exactly those values and ignores `step`, and omitted or `false` draws nothing; a value outside `[min, max]` is dropped rather than pulled to the edge, an end notch sits half its own width inside the track, and every notch is `aria-hidden` and takes no pointer events). Any other `SliderRootProps` (e.g. `name`, `onChangeEnd`, `inverted`, `orientation`) is forwarded via spread. Exported types: `SliderProps`, `SliderDataProps`, `SliderOverrides`. Also exports `SliderField`, the readout field itself (see its own entry below). Factory: `createSlider({ format, ticks })` — curry the formatter and the tick set when the unit and the scale are static decisions. Uses `--sui-accent`, `--sui-accent-rgb`, `--sui-bg-primary`, `--sui-border`, `--sui-border-focus`, `--sui-text-primary`, `--sui-text-secondary`, `--sui-font-family`, `--sui-font-mono`, `--sui-space-2` theme tokens. Use for: a bounded numeric input where the reader wants the value and the position at once — a runway dial, a buffer, a draw amount. For a value axis that also carries named threshold ticks, use `BandRail`.
  ...
  - **`editable` makes the readout a field, and it is `type="text"`.** A number
    ...
    accessible name. `editable` draws a `SliderField` with no prefix and no
    suffix, which is the readout it has always drawn.
  ...
  - **Mounting a Slider under jsdom 30 needs a shim.** Kobalte builds the
    ...
    jsdom's own verdict. The subpath resolves to TypeScript source, not to a
    built file: the shim is test-only, no library module imports it, and
    Vitest transpiles the source itself.
...
- **SliderField** — Atomic (Depth 1). The editable readout of a slider value, on its own. Shares `Slider.css`. `Slider`'s `editable` path draws one with no prefix and no suffix; a caller who draws a `valueLabel` node draws one per figure. **It owns the INTERACTION, never the MEANING**: the focus swap, the width, the commit on Enter and on blur, and the revert on Escape are its; the parse, the clamp and the map back to the value are the caller's. It never reads the slider's own `min`, `max` or `step`, because a `$132.61/mo` figure and the `11%` the track moves have different domains, so one parse would be right for one figure and wrong for the other. Key props: `label` (accessible name — required, and it carries the unit, since the affixes are `aria-hidden`), `value` (`string` — the text at rest; the NUMBER PART only when there is a prefix or a suffix), `editValue?` (`string` — the text the field takes on focus, default `value`; pass the raw number when `value` is formatted), `prefix?` (`string` — static text before the input, such as `"$"`), `suffix?` (`string` — static text after it, such as `"%"` or `"/mo"`), `onCommit` (`(text: string) => void` — the typed text on Enter and on blur; Escape calls nothing), `disabled?`. Exported type: `SliderFieldProps`. No factory and no variants: the field looks one way. Uses `--sui-border`, `--sui-border-focus`, `--sui-bg-secondary`, `--sui-font-mono` theme tokens; it takes the surrounding text colour rather than setting one. Use for: each figure of a `valueLabel` node, so a person types the reading they hold.
  - **The input holds only the number.** `prefix` and `suffix` split the unit away from it and stand beside it as static text. The three parts read as ONE unbroken string: one font, one size, one baseline, no gap. The border, the padding, the hover and the focus sit on the GROUP, not on the input, because the input's own horizontal padding would open a hole between the number and its suffix. The group is a `<label>`, so a press on the `$` or the `/mo` focuses the number — a figure a person reads as one string answers a click anywhere on it. With no prefix and no suffix the input holds the whole formatted string, the way SUI's built-in field always has.
  - **WARNING — the doubled affix.** Nothing type-checks the split between `value` and the affixes. A caller whose own formatter already prints the sign passes `"$1,627.08"`, sets `prefix="$"`, and the field draws `$$1,627.08`. The caller strips every affix out of `value` first. A test that asserts each readout string carries no `"$"` and no `"/"` at all catches whichever affix doubles, whether it is the prefix or the suffix.
  - **WARNING — the caller owns parse, clamp, snap and the empty case.** The field clamps nothing and snaps nothing on purpose, because a `$/mo` figure has a different domain from the `%` the track moves. The consequence is the caller's to handle: on a `$50–$250 step $5` slider, a coach who types `137` gets `137` — a value the control could never produce, and one that looks entirely plausible on screen and in a forecast. The empty field is worse, because it fails SILENTLY: `Number("")` is `0`, so an emptied field commits a real zero rather than doing nothing. Commit only a FINITE parse. `Slider`'s own `editable` path holds that guard; a caller that wires its own `onCommit` inherits none of it. The `valueLabel` showcase does all four steps in one handler.
  - **Width comes from a MIRROR, not from `size`.** The `size` attribute sizes the content box in CHARACTERS, and the UA multiplies it by the AVERAGE character width — so any string whose glyphs run wider than that average overflowed and clipped its last character (`$1,591.32/yr` lost the `r`). The group now holds a hidden span carrying the SAME text in the SAME font, and the input lies over it in the same `inline-grid` cell, so the cell measures the real glyphs. It is exact in a proportional font as well as a monospace one, because nothing counts characters. The input keeps `size="1"` as a FLOOR, otherwise a text input's intrinsic default of 20 characters would win the cell, and the sizer keeps `min-width: 1ch` so an empty field still has a target. The mirror carries the text the input SHOWS, so the field grows and shrinks on every keystroke and does not jump on focus.
  - **Escape is a STATE, not a flag.** Escape blurs the input, and the blur that follows must commit nothing. The field holds `rest | editing | reverting`, so "reverting with a draft" cannot exist.
...
## Dropdown
- **Dropdown** — Atomic (Depth 1). Trigger button plus popover listbox for single selection, with full keyboard support: roving-tabindex options, ArrowUp/Down to open and move focus, Home/End, Escape and click-outside to close (refocusing the trigger), and Tab to leave the widget. Global mousedown/keydown listeners live only while open. Key props: `items` (`DropdownItem[]`, each `{ id: string; label: string; color?: string; shape?: Shape; disabled?: boolean; reason?: string }` — a color alone renders as an indicator dot; adding a `shape` (the chart `Shape` union) renders that shape as the indicator instead, in the trigger and the menu alike, for identities that are double-coded colour *and* shape so they survive small sizes, colour-blindness and greyscale; `disabled: true` renders the row as unavailable — the whole row dims via `--sui-text-muted` plus a lower opacity, so the label and the indicator dim together and the row stays distinct from an item whose own accent is that same muted token — the row reports `aria-disabled="true"` and carries `sui-dropdown__item--disabled`, a click or an Enter/Space on it fires no `onChange` and leaves the menu open, and ArrowUp/ArrowDown/Home/End step over it, so a disabled row never takes the roving tab stop and an all-disabled list simply keeps the focus where it is; `reason` says why the row reads as it does — the row carries it as the native `title` for a mouse user and points `aria-describedby` at a `sui-sr-only` element holding the same text for a keyboard or touch user, and a row without a `reason` emits neither attribute. `reason` is independent of `disabled`, so an available row may carry one too), `value` (`string`, the selected id), `onChange` (`(id: string) => void`), `placeholder` (`string`, default `"Select..."`), `footer` (`JSX.Element`, e.g. an "Add new" action rendered below the list), `size` (`"sm" | "md"`, default `"md"`), `subtle` (`boolean` — trigger looks like read-only text until hovered), `class` (`string`), `trigger` (`(state: DropdownTriggerState) => JSX.Element` — render the trigger yourself), `onDisabledSelect` (`(item: DropdownItem) => void` — fires when the user activates a disabled row by click or Enter/Space; the refusal itself is unchanged, so no `onChange` fires and the menu stays open, and without the prop the pick stays silent as before). The `trigger` slot replaces the whole trigger content (indicator, label and caret) inside a `div[role="combobox"]` that keeps the ARIA wiring and the ArrowUp/ArrowDown handler; `DropdownTriggerState` is `{ open: boolean; selected: DropdownItem | undefined; toggle: () => void; openMenu: () => void; close: () => void }`. The wrapper binds **no** click and carries no `tabindex`, so every click reaches the consumer's element (a caret lands where the user aims) and that element is the focus target — call `toggle` yourself. With the slot present, two behaviours change: `toggle`/`openMenu` open with no option focused (arrow-key opens still focus first/last), so the menu never blurs an input mid-typing; and Escape stops propagation after it closes the menu, so the next Escape reaches the consumer's element (e.g. "revert the name"). Enter stays unclaimed — the wrapper is a `div`, not a submit button, so an `<input>` inside an ancestor `<form>` still submits per browser default. **Dropdown restores the focus itself**: `openMenu` records whatever held focus, and every close path that returns focus (a pick, Escape) gives it back to that element if it is still in the DOM — so the consumer must **not** refocus in its own `onChange`. `open` and `selected` are getters on the state object and the slot is invoked once under `untrack`, so reading them inside the consumer's JSX updates in place instead of rebuilding the consumer's element and losing its focus, caret and IME state. Without `trigger`, the trigger stays the same `<button>` it always was. Use for: an editable name that also picks from a list. Exported types: `DropdownItem`, `DropdownProps`, `DropdownTriggerState`, `DropdownOverrides`, `DropdownDataProps`. Factory: `createDropdown({ size, subtle })`; curried variant **`InlineSubtleDropdown`** (`size:"sm"`, `subtle:true`) — the compact inline picker that reads as plain text until hovered. Use for: single-value selection with optional color indicators and a menu footer action (status pickers, category selectors). `itemAction` (`Component<DropdownItemActionProps>`, `{ item: DropdownItem; selected: boolean }`) draws a component at the END of every row — a trash button, say — as a SIBLING of the option button inside a `div.sui-dropdown__row` (a button cannot hold a button), so its click neither selects the row nor closes the menu; it decides for itself whether to draw anything (e.g. nothing on the selected row). With it the menu is exactly the dropdown's width and long labels ellipsise; without it every row renders byte-identically to before. The menu scrolls past `max-height: 280px`. `onItemDelete` (`(item: DropdownItem) => void`) is the keyboard path to a row's `itemAction`, which Tab cannot reach because Tab leaves the menu: Delete or Backspace on a focused option calls it (the menu does not filter — the consumer decides whether that row may go) and focus moves to the row that takes its place; without it the keys do nothing. Curried variant **`CompactDropdown`** (`size:"sm"`, `subtle:false`) — the small size with the full trigger frame, the form a combo takes inside a dense strip (DirtyComboBox). Theming: `Dropdown.css` owns its colors via `--sui-*` tokens (like Button/NavLink/PopoverMenu), so it adapts to any loaded theme with zero per-theme rules — trigger `color: var(--sui-text-primary)`, opaque menu `background: var(--sui-bg-elevated)` + `border: var(--sui-border-bright)` + plain elevation shadow, `--subtle` transparent-until-hover reading legible text. (The token-driven color layer was added 2026-07-20; before that the component CSS was structural-only, leaving the trigger dark-on-dark and the menu transparent under every theme except `hud`, which ships its own explicit dropdown skin that still wins under hud.)
- **DropdownFitLabel** — Atomic (Depth 1, lives in `Dropdown.tsx`, CSS in `Dropdown.css`). A label for a `trigger` slot that is as wide as the WIDEST of `candidates` (capped at 34ch — about 30 characters of a proportional, letter-spaced face — ellipsised past that), so the trigger does not change width as the selection changes. Every candidate is laid, invisible (`aria-hidden`), into the same inline-grid cell as the visible label. Key props: `label` (`string`), `candidates` (`string[]`, normally every item's label). Use for: a combo whose width must not jump between selections (DirtyComboBox on the payroll-board bench).
  - Example:
    ```tsx
...
- **ThemedNumberInput** — Themed numeric field built on `@kobalte/core/number-field` with stacked increment/decrement triggers. Key props: `value` (`Accessor<number | undefined>`), `onChange` (`(value: number | undefined) => void`), `name`, `label`, `description`, `errorMessage`, `min`, `max`, `step` (default `1`), `size` (`"sm" | "md"`, default `"md"`). Friendly names `min`/`max` map to kobalte's `minValue`/`maxValue`; any other `NumberFieldRootProps` (e.g. `disabled`, `required`, `format`, `formatOptions`, `changeOnWheel`) is forwarded via spread. **It never stretches the whole screen:** the root carries an inline `max-width` derived from the widest value it can show — `max`/`min` rendered by the field's own `formatOptions` (grouping commas, a currency or percent sign, a negative sign, the fraction digits a fractional `step` adds) via `numberFieldChars` + `fieldWidthForChars(chars, 4)` — or, with no `max`, sized for one billion (`"1,000,000,000"`, 13 chars → **12.06rem**). It still shrinks in a narrow column; it only stops growing. When `errorMessage` is set, the field renders in invalid state and suppresses the description. Kobalte emits `NaN` on clear — normalized to `undefined` before `onChange`. `size="sm"` is the toolbar size: a 29px-tall field that lines up with `Button size="sm"` / `Dropdown size="sm"`; the default `md` is 43px and is the tallest control in the family, so a compact row needs `sm` or the number input sets the row height. Uses `--sui-bg-secondary`, `--sui-border`, `--sui-border-focus`, `--sui-accent`, `--sui-accent-rgb`, `--sui-danger`, `--sui-text-primary`, `--sui-text-secondary`, `--sui-text-muted`, `--sui-radius-sm`, `--sui-font-family` theme tokens. Use for: numeric form fields (RPM, counts, thresholds, bounded parameters).
  - Example:
    ...
    />
    ```
- **CurrencyInput** — Curried variant of `ThemedNumberInput` (Depth 1). Owns `CurrencyInput.css`; composes `ThemedNumberInput` + the shared `fieldWidthForChars` util. A money-amount field: the Primitive's stepper + keyboard semantics, plus USD currency masking (`formatOptions: { style: "currency", currency, currencyDisplay: "narrowSymbol" }` — the symbol is always a literal `$`, whatever the viewer's locale; international currency comes later) and a **fixed width capped to the widest expected value** so it never stretches to fill its column. The cap is **derived, not magic** — by default the field is SIZED for one billion: `"$1,000,000,000.00"` is 17 chars → `17 × 0.62rem + 4rem` stepper chrome = **14.54rem** (via `currencyWidthRem`/`fieldWidthForChars`); a larger amount is still accepted, it just fits tightly. Tabular figures keep the masked digits from reflowing as you type. Key props: everything `ThemedNumberInput` takes (`value`, `onChange`, `name`, `label`, `min`, `max`, `step`, `errorMessage`, `description`) **plus** `maxValue?` (drives both the width cap and kobalte's `maxValue` unless `max` is set; unset, the width is sized for `$1B` while the accepted ceiling stays `$10,000,000,000`) and `currency?` (ISO-4217, default `"USD"`). Locale is the app's i18n default (en-US). Exports the `currencyWidthRem(maxValue?)` helper for sharing the exact rem cap. Use for: any money input — pick a smaller `maxValue` for a tighter column. **This is the curried money field; do not configure `ThemedNumberInput` with `formatOptions` at the call site.**
  - Example:
    ```tsx
...
|---|---|---|---|
| `ThemedNumberInput` (every number field) | `max`/`min` as its `formatOptions` render them (default: `1,000,000,000` = 13 chars) + stepper | `12.06rem` (default) | `numberFieldChars` → `fieldWidthForChars(13, 4)` |
| `CurrencyInput` | `maxValue` formatted (default sized for `$1B` = 17 chars) + stepper | `14.54rem` (default) | `currencyWidthRem` → `fieldWidthForChars(17, 4)` |
...
## DirtyComboBox
- **DirtyComboBox** — Composite (Depth 2), owns no CSS. Composes `CompactDropdown` (trigger slot + `itemAction` + `onItemDelete` + `DropdownFitLabel`), `SlideReveal`, `ReservedWidth`, `TagPill`, `TextSublabel`, `Icon`, `SmallGhostButton`/`IconOnlyButton`, `VerticalDivider` and the `ClusterRow`/`TightClusterRow`/`LooseClusterRow` Layout variants. A list of things that can be SAVED or RESET, with dirty detection: `[Baseline] vs [name ▾ │ ✓]  ↺`. PRISTINE (draft equals saved) shows no save and no reset; DIRTY slides the save segment out inside the combo and the reset in beside it (180ms) — the visible frame hugs `[name ▾]` when pristine and GROWS as the save segment slides out, all inside an invisible `ReservedWidth` holding the dirty-state width, so nothing beside the control ever moves and the frame never carries blank padding. The reset sits a loose gap away so it is hard to fat-finger — deliberately no confirm. Every row but the selected one carries a trash (Delete/Backspace on a focused row does the same); the list scrolls; the name is as wide as the LONGEST label (capped near 30 characters, ellipsised), so the combo never jumps when the selection changes. The WORDS are presentational and curried, never passed at a call site: `labels` (`DirtyComboBoxLabels` — `reference` (the static left pill), `versus`, `save`, `reset`, `deleteItem(label)`, and optional `create`, `choose`, `none`) is the only Override. Factory `createDirtyComboBox({ labels })`; the base is not exported. Curried variant **`ScenarioComboBox`** (`"Baseline"`, `"vs"`, `"Save"`, `"Reset to saved"`, `"Delete <name>"`, `"New scenario"`, `"Choose a scenario"`, `"None"`) — thorcasting's payroll scenario strip. Data props (call site): `items` (`DirtyComboItem[]`, `{ id, label, color?, shape?, disabled?, reason? }` — `color`/`shape` draw a swatch on the row AND the selected value; `disabled`/`reason` are Dropdown's refused row with its hover/aria reason), `selectedId`, `view` (`DirtyComboView`, from `dirtyComboModel`), `onSelect(id)`, `onSave()`, `onReset()`, `onDelete(id)` (never called for the selected row or "None"), and the opt-in `onCreate()` (the reset becomes a split `[ ↺ │ + ]` shown ONLY while dirty — it slides in with the ✓ segment; "+" creates, selects and opens the new item's name field) and `onRename(name)` (the name becomes a `fill` `EditableTitle` — a click anywhere from the name's left edge up to the caret opens a text field filling that area, Enter or blur saves, Esc cancels — and the caret alone opens the menu). `labels.none` leads the menu with a "None" row (`DIRTY_COMBO_NONE_ID`). Each is off unless configured. Use for: a scenario/preset/draft picker whose selection carries unsaved edits (thorcasting's payroll scenario strip).
  - Example:
    ```tsx
    import { ScenarioComboBox, dirtyComboViewOf, dirtyComboSelect, dirtyComboSave, dirtyComboReset, dirtyComboRemove } from "solid-ui-components";
    ...
    />
    ```
- **dirtyComboModel** — Pure helper (not a component), the headless observation behind DirtyComboBox. `dirtyComboModel(items, selectedId, draft, saved, equals = dirtyComboEqual) => DirtyComboView` = `{ dirty, canSave, canReset, canCreate, deletableIds, widthCh, selectedLabel, none, canRename }` (`widthCh` is the longest label in characters, clamped to `[DIRTY_COMBO_MIN_WIDTH_CH, DIRTY_COMBO_MAX_WIDTH_CH]` = `[4, 30]` — a headless reading; the pixel width comes from `DropdownFitLabel`). `dirtyComboEqual` is structural equality over plain data. Store transitions over a `DirtyComboStore<C>` (`{ items: DirtyComboSavedItem<C>[]; selectedId; draft }`): `dirtyComboViewOf(store)`, `dirtyComboSelect(store, id)` (loads that item's saved config — an unsaved edit on the one being left is DISCARDED, no prompt), `dirtyComboSave(store)`, `dirtyComboReset(store)`, `dirtyComboRemove(store, id)` (refuses the selected row), `dirtyComboCreate(store, { id, label, config? })` (appends and selects; refused while pristine; `config` defaults to the DRAFT, so "new" is a save-as-new), `dirtyComboRename(store, name)` (the selected item; trimmed; never dirties). `dirtyComboSelect` refuses a disabled row; selecting `DIRTY_COMBO_NONE_ID` compares nothing (never dirty, nothing to rename). `dirtyComboWidthCh(items)` and `dirtyComboUniqueLabel(items, base)` (`base`, `base 2`, …) are exported too.

## SlideReveal
- **SlideReveal** — Atomic (Depth 1). Owns `SlideReveal.css`, no component imports. Slides its children in and out horizontally: its box grows from zero to the children's own width (a `0fr ↔ 1fr` grid column, nothing measured in JS) while they fade, in 180ms ease-out; instant under `prefers-reduced-motion`. Because the BOX grows, a frame around it grows with it (DirtyComboBox's combo border widens as the save segment slides out). **The rule (Peter, 2026-09-24): an animation must hold enough space that it never shifts the layout of other components — and hold it INVISIBLY, with an outer element, so the visible thing carries no blank padding.** So pair SlideReveal with `ReservedWidth` around the thing it lives in; the animation then happens inside that reservation. (A clip-only SlideReveal that always held its own width kept neighbours still but left a blank area inside the frame — rejected.) Collapsed children stay MOUNTED, marked `inert` + `aria-hidden`. Key props: `when` (`boolean`), `children`. Use for: actions that appear only in a state.

## ReservedWidth
- **ReservedWidth** — Atomic (Depth 1). Owns `ReservedWidth.css`, no component imports. **The invisible outer element of Peter's animation rule:** it lays `children` (the live render) and `widest` (the same thing drawn in its widest state) into ONE inline-grid cell, so the box is always the wider of the two and never changes size — neighbours never move — while the live content stays at the start of the cell and keeps hugging itself. `widest` is `visibility: hidden`, `inert` and `aria-hidden`: it reserves, draws nothing and is unreachable (the live content comes first in the DOM, so a query finds it first). Key props: `children`, `widest` (`JSX.Element`). Use for: anything whose width animates or swaps between states (DirtyComboBox; a SlideReveal beside a label).
...
- **LooseClusterRow** — Curried `Row` (`align:center; gap:lg`, 16px): a centred cluster at the LOOSE gap, for an action that must sit out of fat-finger reach of its neighbour. DirtyComboBox keeps its reset this far from its save. Use `ClusterRow` (sm) for ordinary groupings.
- **Box** — Flex child with grow/shrink control. Key props: `grow`, `shrink`. Use for: controlling flex item sizing.
- **HalfFillColumn** — Curried `Stack` taking an EQUAL share of a proportional split (`flex:1 1 0; min-height:0`). Two halve their parent, three third it. The `0` basis is the load-bearing part and the reason this is not `ClipFillColumn`: a basis of `auto` measures each child's CONTENT first, so a tall chart beside a short one takes the lion's share and "equal halves" silently becomes "proportional to content". Use for: two stacked charts that must get the same room whatever they contain.
- **GrowFillBox** — Curried `Box`, the COLUMN-context sibling of `GrowBox`: fills the space its parent column has left and may shrink past its content (`flex-grow:1; flex-basis:0%; min-height:0; min-width:0`). `GrowBox` carries `min-width:0` and nothing for the other axis because it was written for a row, and in a column that omission is decisive — a flex item's automatic minimum size is its content, so a `GrowBox` holding a chart that reports an 800px intrinsic height refuses to shrink into a 150px cell and overflows it silently. `min-height:0` is what lets the cell win. Use for: the growing region of a card whose header keeps its own height.
- **FixedHeightBox** — Curried `Box` whose height is STATED rather than left to a parent fill/flex context (`height:200px; min-height:0; overflow:hidden`), where 200px is `ScrubChart`'s own `DEFAULT_CHART_HEIGHT` — reused as a data import so a card can match a sibling chart's height without a caller writing `height` inline. `min-height:0` lets a tall child shrink into the stated box instead of blowing it out; `overflow:hidden` enforces the bound. Not `GrowFillBox`, which fills whatever space its OWN parent has left rather than a fixed value. Use for: a card whose height must match a sibling chart's height exactly — put a fill child (`GrowFillBox`, or a chart with `chartHeight="fill"`) inside it.
- **ViewportColumn** — Curried `Stack` that fills a parent of DEFINITE height (`fill` → `height:100%; min-height:0`). The bridge between a block container sized in CSS (a page or bench frame, typically `calc(100vh - N)`) and the Layout vocabulary. **Not interchangeable with `FillColumn`**, which bakes `flex:1` and therefore does nothing inside a block parent — a column that looks right in a flex page silently grows to its content in a block one. A variant rather than a call-site prop because `fill` is a Stack Override, locked at definition time. Use for: the outermost column of a viewport-height layout.
- **GrowCenterColumn** — Curried `Stack` that fills the height its parent column has left and CENTRES its child in it (`flex:1 1 0; min-height:0; justify:center`). The sibling of `GrowFillBox` for content that should not be stretched: a chart that fills its box wants `GrowFillBox`, but an INSTRUMENT that keeps its own aspect — a dial, a gauge — grows until it is width-bound and then has height to spare, and pinning it to the top of a tall card reads as a mistake. Use for: a gauge or dial in a card taller than the instrument's aspect allows it to fill.
- **FillWrapRow** — Curried `Row` that FILLS its parent column's height and stretches its cells to match (`flex:1; min-height:0; wrap; gap:sm`). Closes the gap the other wrapping rows leave: a flex ROW inside a flex COLUMN takes its height from its CONTENT, because the column's `align-items: stretch` governs the cross axis — which for a column is the WIDTH. So a row of cards in a sized column sits at content height and leaves the rest of the column empty. `flex:1` claims the height and `min-height:0` lets it shrink again. Not `GrowWrapRow`, which grows along a ROW and centres its items so cards float at their own heights; not `PaneRow`, which fills but does not wrap. Use for: the detail region of a viewport-height dashboard, where two cards must fill the space below the overview band and still stack when the row runs out of width.
- **MajorPaneBox** — Curried `Box` pinned to 60% of its row (`width:60%; flex-shrink:0; min-width:0`), beside a companion that fills the remaining 40%. DEFINITE on purpose: the obvious spelling for "as wide as its content needs, capped" is `width:max-content; max-width:<n>%`, and that is **unsafe for any child that measures itself to decide how much content to render** (a paging row, a virtualised list) — the measurement changes the content, the content changes the `max-content` box, and the box changes the measurement. It does not reliably spin; it can settle one item lower and oscillate between two widths depending on which frame the resize arrived in, which reads as a rendering bug and does not reproduce on demand. A definite share breaks the cycle. Use for: a wide instrument panel beside a readout that still needs room to be legible.
- **FillPaneRailGrid** — Curried `Grid`: a fluid PANE beside a RAIL held to a stated width, filling its parent column's remaining height (`grid-template-columns: minmax(0, 1fr) 292px; grid-template-rows: minmax(0, 1fr); flex:1; min-height:0; gap:sm`). 292px is `RateGauge`'s own `NATURAL_GAUGE_WIDTH` — a data import, the way `FixedHeightBox` imports `DEFAULT_CHART_HEIGHT` — being the smallest width at which the gauge's label column is unclamped and its ring still reaches full size. Two things break without it. **A percentage is not a width**: a `MajorPaneBox` + `GrowFillBox` row hands the minor cell a share, so an instrument in it measured 438px at a 1439px viewport and 262px at a 1000px one, and below its natural width a gauge does not ellipsize — it starves the dial to leave the words room. **`minmax(0, …)` is not `1fr`**: `1fr` is `minmax(auto, 1fr)`, whose min sizing function is `auto`, so a grid item keeps its content-based automatic minimum and a self-measuring pane can push the stated track off. With a `0` min the item has no such minimum and is shrunk to the track — which is what a flex row cannot do, its cross-size stretch being floored by content (the License Board's bottom band measured 468px inside its own 393px half before this). The row track carries the same `minmax(0, …)` for the same reason on the other axis, and it is load-bearing: drop it and the implicit row is auto-sized to content and the overflow returns. Not `FillWrapRow`, which fills but WRAPS — a wrapped rail drops below its pane and takes the row's height with it; not `PaneRow`, where both sides are fluid; not `HalfFillColumn`, which splits proportionally rather than stating a width. Named for the shape, not the instrument: anything wanting a constant width beside a fluid pane belongs in it. Use for: a board's bottom band — a pane of controls beside one instrument that must be the same size on every screen.
- **AnchorBox** — Curried `Box`: the CONTAINING BLOCK for an absolutely-positioned affordance (`position: relative`), no size and no flex of its own. **AnchorFillBox** — the same, filling a parent of definite height (`position: relative; height: 100%; min-height: 0`) so a `chartHeight="fill"` chart under a corner control measures the cell rather than 0px. **TopRightOverlayBox** — the affordance itself, pinned to the top-right corner of its anchor (`position: absolute; top: 4px; right: 4px; z-index: 2`). Use for: a chart with one page-level corner button (a y-axis shrink) or an edge-pinned legend.
...
## OverflowNav
- **OverflowNav** — Pure Composite (Depth 2). Composes `Row` (Layout Primitive) + `NavLink` (Atomic Primitive) + `PopoverMenu` (Atomic Primitive). Owns zero CSS and zero inline `style={}` (other than the `style={props.style}` passthrough on the outer `Row`). Horizontal nav row that automatically collapses items that don't fit horizontally into a trailing kebab (⋮) `PopoverMenu`, re-evaluated on container resize via `ResizeObserver` (rAF-debounced). Items render as full `NavLink`s while they fit; spilled items render as menu entries inside the kebab popover and activate via their `onClick` (or `href` fallback). Measurement strategy: on mount and whenever `items` change, all items render inline for one frame so the component can snapshot each item's natural `offsetWidth`; on subsequent resizes the cached widths drive the trim. A ~48px budget is reserved for the kebab trigger so the boundary case ("fits without kebab" ↔ "fits with kebab") doesn't oscillate. Key props: `items` (`OverflowNavItem[]` — `id`, `label`, optional `href`, `active`, `color` (`accent`|`warning`|`danger`|`success`), `badge`, `onClick`, `closable`), `overflowItems` (`OverflowNavItem[]`, default none — items that ALWAYS live in the kebab regardless of width, listed after width-spilled ones; the kebab shows whenever either list is non-empty and its reserve always applies; e.g. closed tabs the reader can re-open), `onClose` (`(id) => void` — a `closable: true` item renders a trailing icon-only close `Button` (×, `aria-label="Close <label>"`) beside its `NavLink`, wrapped in `TightNoShrinkClusterRow`; clicking it calls `onClose(id)` and never the item's `onClick`; no `onClose` → no close buttons; kebab rows never close), `gap` (`xs`|`sm`, default `"sm"` — forwarded to `Row`; deliberately narrower than `Row`'s own `xs`|`sm`|`md`|`lg`, since no shipped consumer has asked for the wider steps here and its `gapPx()` overflow budget only accounts for these two), `align` (default `"center"`), `class?`, `style?`. Exported types: `OverflowNavProps`, `OverflowNavItem`. Inherits theme styling from `NavLink` (`--sui-*` / theme tokens already wired through `nav-link*` classes) and from `PopoverMenu` (kebab trigger + dropdown panel). Use for: top nav bars, breadcrumb-style tab rows, any horizontal nav list that must adapt to a constrained container width without wrapping. Fold (G16): natural widths are re-measured on every container resize, on item/label/badge changes and once fonts are ready; a 0px container folds every item into the kebab; the rule is the pure `overflowNavVisibleCount` in `fold.ts`.
  - Example:
    ```tsx
...
- Curried variants (always prefer these over configuring `Text` directly):
  ...
  - **VerticalAxisTitle** — Sublabel `<span>` written bottom-to-top (`writing-mode: vertical-rl` + `rotate(180deg)`, nowrap). Use for: a chart's y-axis title naming the axis and its units ("Salary ($)", "Hours"), along the plot's left edge — `ChartFrame`'s y-title rail.
  ...
  - **EllipsizedHudCaption** — Single-line truncating uppercase/tracked HUD caption (11px / 600 + 0.5px tracking + nowrap + ellipsis, `<span>`, `color: inherit`). Use for: the label at the end of a leader line in an SVG instrument, inside a `<foreignObject>` sized to the callout column, paired with a `Tooltip` carrying the untruncated text. The `inherit` is load-bearing — the caption takes the tone of the callout its SVG parent already carries.
  - **EllipsizedNodeLabel** — Single-line truncating node label (12px / 600, `<div>` at `width: 100%`, `text-align: center`, nowrap + ellipsis). Needs no flex parent — the fixed-width slot around it is the constraint. Use for: the title line of a diagram node box, inside a `<foreignObject>` clamped to the box, paired with a `Tooltip` carrying the untruncated text. **Not interchangeable with `EllipsizedHudCaption`** (ruled 2026-09-16): that one is uppercase, tracked, 11px and colour-inheriting for an SVG callout; this one is plain-case, centred, full-width and paints in the `label` colour. Two roles, two variants, no alias.
...
## FullscreenBox
- **FullscreenBox** — Atomic (Depth 1). A box that takes the whole viewport and gives it back WITHOUT REMOUNTING its children: the SAME element toggles between in-flow and `position: fixed; inset: 0` (class `sui-fullscreen-box--on`, `data-fullscreen`), with NO portal — `Modal`'s fullscreen portals and so remounts, which costs a chart its held axis marks, fit mode, scroll and tweens. In fullscreen the box is a flex column whose first child fills it, so a `chartHeight="fill"` chart takes the viewport. Props: `fullscreen?` / `onFullscreenChange?` (controlled, or owned starting in flow; every change reported), `renderCorner?: (ctx: { fullscreen, toggle }) => JSX.Element` (the top-right corner slot; default an `IconOnlyButton` with `fullscreen` / `fullscreen-exit` (corner brackets, never arrows), named "Full screen" / "Exit full screen"; return `null` for none), `escapeCloses?` (default `true`; an override — curry it with `createFullscreenBox`). `fill?` (override, default `false`): in flow the box FILLS a parent of definite height (`height: 100%; flex: 1 1 0; min-height: 0`, a flex column handing it to its first child) instead of sizing to content — curried as **`FillFullscreenBox`**, which `FillChartFrame` uses. Escape is a document listener that exists only while open and ignores an Escape something inside already handled (`defaultPrevented`, e.g. the ScrubChart range editor). z-index 900: above content, below `Modal` (1000) and `Tooltip`/`Combobox` (1100). Caveat: an ancestor with `transform`/`filter`/`perspective`/`contain: paint` becomes the containing block. Use for: a chart (or any stateful view) that must go full screen and come back as the same instance.
