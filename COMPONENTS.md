...
- **MinorFillColumn / MajorFillColumn** — Curried `Stack` pair for a PROPORTIONAL vertical split: `flex:30` and `flex:70`, so two siblings divide whatever height their parent has. The numbers are flex GROW factors rather than percentages, which is the point — a percentage height needs a definite height on every ancestor between it and the frame and resolves to `auto` in silence when one link is missing, whereas grow factors work against a parent sized by a `calc()` or a viewport unit. A third sibling that should keep only its intrinsic height (a toolbar, a control row) takes neither. Use for: an overview band above a detail region in a viewport-height dashboard.
