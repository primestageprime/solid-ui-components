...
- **WidePaneBox** — Curried `Box` pinned to 80% of its row (`width:80%; flex-shrink:0; min-width:0`), beside a companion that takes the rest. DEFINITE on purpose: the obvious spelling for "as wide as its content needs, capped" is `width:max-content; max-width:80%`, and that is **unsafe for any child that measures itself to decide how much content to render** (a paging row, a virtualised list) — the measurement changes the content, the content changes the `max-content` box, and the box changes the measurement. It does not reliably spin; it can settle one item lower and oscillate between two widths depending on which frame the resize arrived in, which reads as a rendering bug and does not reproduce on demand. A definite share breaks the cycle. Use for: a wide instrument panel beside a fixed-width readout.
