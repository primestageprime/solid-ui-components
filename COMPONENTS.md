...
- **ToastPrimitive** — Re-export of the raw kobalte `Toast` namespace (`Root`, `Title`, `Description`, `CloseButton`, `ProgressTrack`, `ProgressFill`, `Region`, `List`). Escape hatch for callers that need multi-region layouts or direct control over sub-components.
- **showToast(input)** — Imperative helper. Returns `{ id: number, dismiss: () => void }`. Accepts the same shape as `ToastProps` minus `toastId`.
- **toaster** — Re-export of kobalte's raw `toaster` (for `update`, `clear`, `promise` use cases beyond `showToast`).
  - Example:
    ```tsx
