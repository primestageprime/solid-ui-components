...
## Icon
- **Icon** — SVG icon component with 30 named icons across 6 groups (status, navigation, data, time, actions, UI/cache; includes `pause`, `agent`, `dependency`). Key props: `name` (e.g., `check`, `warning`, `chevron-down`, `search`, `spinner`), `variant` (`outline`|`solid`), `size` (`xs`|`sm`|`md`|`lg`|`xl`). `variant`/`size` are Overrides — curry them via `createIcon`; **`InlineMetaIcon`** (outline, xs) is the shipped variant for icon-beside-sublabel meta rows. Use for: all iconography. Spinner icon auto-animates.
...
## SplitQueueList
- **SplitQueueList** — **DEPRECATED, removed in the next major.** A compile shim over `ProgressionQueue` (see below), kept for one release so existing call sites keep working. It is **not pixel-identical** — the merged component draws its own chrome, so the rendered result is `ProgressionQueue`'s, not the old two-pane seam. `topCapRows` maps to the resolved section's `capRows`; `topOnly`, `topFloorRows`, `animationMs`, and `rowHeight` are accepted but **ignored** on the animated path (the merged component measures rows, collapses empty sections, and owns its own motion). `static` mode is unaffected — it still delegates to `StaticSplitLayout` below, which is **not** deprecated and keeps using both `rowHeight` and `topCapRows`. Migrate new call sites straight to `ProgressionQueue`: declare `sections` and bucket `items` with `bucketOf` rather than relying on this mapping. Full usage guide: `src/components/SplitQueueList/README.md`.
...
## ProgressionQueue
- **ProgressionQueue** — Layout-tagged Primitive (EXEMPT-AS-LAYOUT, STYLE_GUIDE § Layout Purity). Owns `ProgressionQueue.css`: the weighted water-fill sizes each section in JS, which no CSS rule can express. The library's single queue component — supersedes `SplitQueueList` (see `docs/adr/0004-one-queue-component-and-the-motion-seam.md`). N always-present sections stacked as one full-height bar, bucketing a **flat `items` list** through its lifecycle as a **progression** (e.g. terminal-happy on top, terminal-unhappy in the middle, transient at the bottom). **Every section is shown at all times with its count.** Generic over `T`; the consumer owns the data and row content — `sections: ProgressionSection[]` (top → bottom, each `{ key, label, tone, weight?, selectable?, emptyLabel?, capRows? }`), `items: T[]`, `bucketOf: (item) => sectionKey`, `keyOf`, `renderItem`. **A move is an item whose `bucketOf` result changed** — one atomic `items` mutation plays a transfer animation between whichever two sections are involved, in either direction; there is no `resolve()`/`unresolve()` pair. **Sizing (ruled 2026-07-22 — a weighted water-fill measured in JS; pure CSS can't express it):** an **empty** section collapses to just its summary line (label + count, or `emptyLabel` copy if given); a **populated** section **shrink-wraps** to its content; when the populated sections **overflow** the available height they share it by `weight`. A section's `capRows` caps its natural height at that many rows, beyond which its body scrolls — unlike `SplitQueueList`'s old top pane, **a capped section never grows past its cap** to absorb slack from a short neighbour. The bar fills its parent's height (drop it in a definite-height flex column) or an explicit `height` in px. **Chrome is thematically NEUTRAL** — uniform border + default header text; the only role color is a small **dot** beside each section label (`tone` → theme var). **Selection is controlled:** pass `onSelect(key)` / `selectedKey` for click-to-select; a selected row shows only an inset accent bar, **never a background fill** — hover owns the fill, so a selected-and-unhovered row stays fully readable. **Roving-focus keyboard nav (controlled):** `focusedKey` / `onFocusChange`; Up/Down/Home/End traverse every row across all sections in render order with no wrap, Enter/Space activates. Only **interactive** rows (a global `onSelect`, or a `selectable` section in select mode) join the tab sequence — a read-only row still renders but is skipped by arrows and never takes the tab stop. **Select mode is on iff `checkedKeys` is present — there is no `selectMode` prop.** An empty `Set` means "mode on, nothing checked." It applies only to sections marked `selectable: true`; rows elsewhere keep selecting on click even while select mode is on. `onToggleCheck(key, { shift, meta })` fires instead of `onSelect` for a checkable row's click/Enter/Space; never both. `scrollToKey` reacts on change to scroll a row into view (also used internally to reveal a row after it transfers). Key props (14 total): `sections`, `items: T[]`, `bucketOf: (item: T) => string`, `keyOf: (item: T) => string`, `renderItem: (item: T) => JSX.Element`, `selectedKey?`, `onSelect?: (key: string) => void`, `focusedKey?`, `onFocusChange?: (key: string | null) => void`, `checkedKeys?: ReadonlySet<string>`, `onToggleCheck?: (key: string, modifiers: { shift: boolean; meta: boolean }) => void`, `scrollToKey?`, `height?` (omit to fill parent), `class?`. The pure sizing core is exported as `allocateHeights(input): number[]` (with the `AllocateInput` type) for callers who need the water-fill math outside the component. No factory — the data/sections are per-call, so the base component is already curried. Motion (the transfer animation) is curried, not a prop — see the README's Motion section and the ADR for the swappable choreographer seam. Use for: a compliance/triage sidebar where items live in a few lifecycle states you always want visible with counts (fortnight vessel calls by compliance, review queues split happy/unhappy/pending, transaction categorization). **Full usage guide: `src/components/ProgressionQueue/README.md`.**
  - Example:
    ```tsx
...
## NotificationCenter
- **NotificationCenter** — Composed (Depth 3). A bell trigger with a count badge that opens a dropdown panel of notifications. Owns a minimal `NotificationCenter.css` for overlay chrome only — the trigger's transparent styling and its corner badge/spinner anchoring — a deliberate structural exception, the same one `Fab` takes (positioning geometry no atomic variant expresses). All content composes `Icon` (bell + spinner), `CountBadge`, `PopoverSurface` (panel) and `CompactSurface` (per-item card), the `TightStack`/`SpreadRow`/`ScrollColumn` layout variants, `TextValue`, `MutedBody`, `TextButton`, and `NavLink`. Each item follows the three-line card canon (see `docs/agents/design-decision-tree.md` › Card formats): a non-interactive `CompactSurface` box → `TightStack` → title row (`SpreadRow`: `TextValue` left, trailing transient spinner), a `MutedBody` detail sandwich, and the accent action. Beyond that chrome CSS, the only inline styling is the dynamic panel position — a `Portal` + `fixed` wrapper measured from the trigger (the sanctioned overlay carve-out of Layout Purity). **Router-agnostic and domain-agnostic**: the consumer supplies `items` and does the navigating inside `onAction` — the component never imports a router and needs no consumer CSS. Supports both **uncontrolled** (manages its own open state) and **controlled** (`open` + `onOpenChange`) use; a controlled `open` lets the consumer auto-open on new activity. Closes on click-outside and Escape. Each item's action renders as a `NavLink` when it has an `href` (plain left-click SPA-navigates via `onAction`; modifier/middle-clicks fall through so new-tab gestures still work) or as an accent `TextButton` when it doesn't. Items marked `transient` show a spinner instead of an action and are excluded from the badge count. Key props: `items` (`NotificationItem[]`), `badgeCount?` (number — defaults to the count of non-transient items), `busy?` (boolean — overlays a spinner on the bell and announces "Working…"), `open?` (boolean — presence switches to controlled mode), `onOpenChange?` (`(open: boolean) => void`), `onAction?` (`(item: NotificationItem) => void` — fires on activation, then the panel closes), `emptyLabel?` (string, default `"You're all caught up."`), `label?` (string, default `"Notifications"` — used as the trigger's `aria-label` and the panel's landmark label), `badgeTone?` (`accent`|`neutral`|`danger` — **RESERVED**: present in the type, no v1 visual, per the #2 Rule). Exported types: `NotificationCenterProps`, `NotificationItem` (`id`, `title`, `detail?`, `action?`, `transient?`, `tone?`), `NotificationAction` (`label`, `href?`). Note: `NotificationItem.tone` (`info`|`task`|`warning`) is likewise **RESERVED** — carried on the type with no v1 rendering. Accessibility: the trigger carries `aria-label`/`aria-haspopup`/`aria-expanded` and `aria-busy` while working; a visually-hidden `.sui-sr-only` `aria-live="polite"` region announces transient item titles (or "Working…"); the panel is a named `<section aria-label>`, which is an implicit region landmark. Theming: the trigger uses `--sui-text-secondary`; all other color/spacing comes from the composed atoms and their tokens. Use for: a global notification/inbox bell in an app header, an activity feed with per-item follow-through actions, surfacing background-task progress (transient rows + `busy`).
