...
- **BuilderBoardBelowChart** — Composite (Depth 2). The BuilderBoard for a page under the app shell's persistent chart: panel **A** is the SHELL's chart (one instance that never remounts across pages), so this board draws **B** over **C | D** only. C|D keeps `BUILDER_BOARD_CD_SHARE` (half) of the space below the tab bar; B takes the remainder of the top half; when the chart the app asks for would leave B under `BUILDER_BOARD_B_MIN_HEIGHT` (156px: a title row + `sm` gap + a 120px plot), the CHART gives way. The C|D height is not a proportion of the board's own box, so it arrives as `lowerHeight` from the pure `builderBoardBelowChart({ viewport, tabBarH, chartH, legendH, gaps?: { chartToB? }, rail? })` (the B-to-C|D gutter is the board's own baked `sm` and is not an input) → `{ chartH, chartGaveWay, b, c, d, lowerHeight }` (viewport coordinates) — the app renders the chart at the returned `chartH` and passes `lowerHeight` here, so the rects the function states are the rects drawn. `observeBuilderBoardBelowChart(input)` prints them as a text table (the headless observation). Same vocabulary as BuilderBoard: no CSS, Layout/Surface variants only (`ViewportColumn` › `HalfFillColumn`(B) + `NoShrinkColumn`(C|D, the one measured height) › `FillPaneRailGrid`). Curry with `createBuilderBoardBelowChart({ rail })`. Chart height POLICY (share of the window, min) is the app's, not this module's.
