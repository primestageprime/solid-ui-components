...
| spinner over content while it loads / saves | `BusyOverlay` | Not a bare spinner and not a progress bar: it anchors to a positioned parent and dims the content underneath. `InlineChartErrorOverlay` is its failure-state sibling; `AsyncProgress`/`StackedProgressBar` are for when a real percentage exists. |
| image / photo / thumbnail display | `FramedImage` (curried: `SmallSquareThumbnail`/`MediumSquareThumbnail` for a cropped square, `ContainedPhoto` for a full letterboxed detail view, `ContainedPhotoOnDark` for the same on a fixed near-black photo stage) | There is no "image component" and there was never meant to be one — a plain `<img>` IS the correct content, framed by component-owned CSS (`overflow:hidden` + `object-fit`), same pattern `MediaCard` already used internally for its own thumbnail slot before this extracted it into a standalone primitive. Don't conclude "SUI needs an image component" from a name search turning up nothing; check whether an existing composite (here, `MediaCard`) already solved the FRAMING problem internally first. |
