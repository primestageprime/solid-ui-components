...
// Sub-column distribution (below : above) — 2:1, 1:2, 2:2, 3:1, 1:3, 1:0, 0:1.
const ELEMENT_ITEMS: ElementItem[] = [
  // earth · dog (2:1)
  { id: "e-dog-b1", area: "earth", focus: "dog",     position: "below", label: "Accumulate Wealth" },
  { id: "e-dog-b2", area: "earth", focus: "dog",     position: "below", label: "Guard Territory" },
  { id: "e-dog-a1", area: "earth", focus: "dog",     position: "above", label: "Found Industry" },
  // earth · pig (1:2)
  { id: "e-pig-b1", area: "earth", focus: "pig",     position: "below", label: "Wallow in Mud" },
  { id: "e-pig-a1", area: "earth", focus: "pig",     position: "above", label: "Build Sty" },
  { id: "e-pig-a2", area: "earth", focus: "pig",     position: "above", label: "Forage Truffles" },
  // air · bird (2:2)
  { id: "a-brd-b1", area: "air",   focus: "bird",    position: "below", label: "Find Updraft" },
  { id: "a-brd-b2", area: "air",   focus: "bird",    position: "below", label: "Gather Twigs" },
  { id: "a-brd-a1", area: "air",   focus: "bird",    position: "above", label: "Soar on Thermals" },
  { id: "a-brd-a2", area: "air",   focus: "bird",    position: "above", label: "Sing at Dawn" },
  // fire · dragon (3:1)
  { id: "f-drg-b1", area: "fire",  focus: "dragon",  position: "below", label: "Hoard Gold" },
  { id: "f-drg-b2", area: "fire",  focus: "dragon",  position: "below", label: "Brood on Eggs" },
  { id: "f-drg-b3", area: "fire",  focus: "dragon",  position: "below", label: "Sleep Deeply" },
  { id: "f-drg-a1", area: "fire",  focus: "dragon",  position: "above", label: "Breathe Flame" },
  // fire · phoenix (1:3)
  { id: "f-phx-b1", area: "fire",  focus: "phoenix", position: "below", label: "Burn to Ash" },
  { id: "f-phx-a1", area: "fire",  focus: "phoenix", position: "above", label: "Rise Reborn" },
  { id: "f-phx-a2", area: "fire",  focus: "phoenix", position: "above", label: "Ignite the Sky" },
  { id: "f-phx-a3", area: "fire",  focus: "phoenix", position: "above", label: "Carry Embers" },
  // water · snake (1:0)
  { id: "w-snk-b1", area: "water", focus: "snake",   position: "below", label: "Bask in Sun" },
  // water · fish (0:1)
  { id: "w-fsh-a1", area: "water", focus: "fish",    position: "above", label: "Bask in Water" },
];
...
const ElementsGridStep: Component = () => {
  const layout = createMemo(() => {
    const byArea = new Map<
      ElementArea,
      Map<string, { above: ElementItem[]; below: ElementItem[] }>
    >();
    ...
    const subCols: {
      area: ElementArea;
      focus: string;
      slot: { above: ElementItem[]; below: ElementItem[] };
    }[] = [];
    const areaSpans: { area: ElementArea; startSubCol: number; span: number }[] = [];
    let subColIdx = 1;
    ...
    // Vsep grid-column indices that fall between two different areas — these
    // show in the header row. Interior vseps within an area are covered by
    // the header's grid-column span, so they don't appear up top.
    ...
    // Vsep grid-column indices between every sub-column (used in cell + label rows).
  ...
  // anchor: "bottom" pushes cards down (above-the-line), "top" pushes them up
  // (below-the-line) so they hug the focus-label band.
  const CellStack: Component<{ items: ElementItem[]; anchor: "top" | "bottom" }> = (p) => (
    <div
      style={{
        height: "100%",
        display: "flex",
        ...
        "align-items": "center",
        "justify-content": p.anchor === "bottom" ? "flex-end" : "flex-start",
        gap: "12px",
        padding: "12px 8px",
      }}
    >
    ...
    <MockBaseline
      ...
      detail={
        <SpacedStack>
          ...
          <MutedBody>
            10 items across 4 areas (earth · air · fire · water). Each sub-column
            is a focus within its area; above-the-line solutions sit on top,
            below-the-line needs at the bottom, with the focus label between them.
          </MutedBody>
          {(() => {
            const L = layout();
            const N = L.subCols.length;
            const totalCols = 2 * N - 1;
            const colTemplate = Array.from({ length: totalCols }, (_, i) =>
              i % 2 === 0 ? "minmax(120px, 1fr)" : "1px",
            ).join(" ");
            // Rows: header | hsep | above | hsep | label | hsep | below
            const rowTemplate =
              "auto 1px minmax(80px, 1fr) 1px auto 1px minmax(80px, 1fr)";
            const sep: JSX.CSSProperties = { background: "#fff" };
            return (
              <div
                style={{
                  display: "grid",
              ...
              >
                {/* Row 1 — area headers (span their sub-cols + interior vseps). */}
                ...
                {/* Vseps between areas in the header row. */}
                <For each={L.areaBoundaryCols}>
                  {(col) => (
                    <div style={{ ...sep, "grid-row": "1", "grid-column": `${col}` }} />
                  )}
                </For>
                {/* Row 2 — full-width hsep under headers. */}
                <div style={{ ...sep, "grid-row": "2", "grid-column": "1 / -1" }} />
                ...
                <For each={L.interSubColVseps}>
                  {(col) => (
                    <div style={{ ...sep, "grid-row": "3", "grid-column": `${col}` }} />
                  )}
                </For>
                {/* Row 4 — full-width hsep above the label band. */}
                <div style={{ ...sep, "grid-row": "4", "grid-column": "1 / -1" }} />
                {/* Row 5 — focus label band. */}
                <For each={L.subCols}>
                  {(sc, i) => (
                    <div
                      style={{
                ...
                <For each={L.interSubColVseps}>
                  {(col) => (
                    <div style={{ ...sep, "grid-row": "5", "grid-column": `${col}` }} />
                  )}
                </For>
                {/* Row 6 — full-width hsep below the label band. */}
                <div style={{ ...sep, "grid-row": "6", "grid-column": "1 / -1" }} />
                ...
                <For each={L.interSubColVseps}>
                  {(col) => (
                    <div style={{ ...sep, "grid-row": "7", "grid-column": `${col}` }} />
                  )}
                </For>
              </div>
            );
