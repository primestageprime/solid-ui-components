// SlotCard — the whole curried template family, each rendered at three widths
// so the priority-based slot dropping is visible rather than described.
// (Promoted out of the retired slot-cards workshop bench, 2026-07-27.)
...
const TEMPLATES: TemplateSpec[] = [
  ...
  { name: "TitleDotsMeta", Card: TitleDotsMeta, signature: "[name·1][dots·2] / [string·2][date·2][duration·2]", props: {},
    values: { name: { text: "Pacific Trader", icon: "⛴" }, dots: [{ label: "NOx", tone: "danger" }, { label: "ROG", tone: "success" }], string: "xbox5-1", date: "2025-12-01", duration: "24.0h" } },
  { name: "TitleMetaCount", Card: TitleMetaCount, signature: "[name·1] / [string·3][count·2]", props: {},
    values: { name: { text: "Themis Leader", icon: "⛴" }, string: "2025-12-01 · 24.0h", count: { n: 7, glyph: "⚠" } } },
  { name: "PickCard", Card: PickCard, signature: "[cornerBadge] [text·1] [remove]", props: { corner: "1", onRemove: noop, onSelect: noop },
...
const ResponsiveStrip: Component<{ spec: TemplateSpec }> = (props) => (
  <ContentStack>
...
// ~40ch; wider caps (in px) reveal more of the name.
const MAX_WIDTHS = [320, 480, 640, 960];

const MaxWidthDemo: Component = () => (
  <ContentStack>
    <SubsectionTitle>TitleStatus — card max-width (name caps ~40 chars by default)</SubsectionTitle>
    <TextSublabel>
      Same long name; the card stops growing at its max-width and the name
      ellipsises. Wider caps show more of the name.
    </TextSublabel>
    <WrapRow>
      <For each={MAX_WIDTHS}>
        {(mw) => (
...
export const SlotCardShowcase: Component = () => (
  <div class="component-section component-section--full">
    <h2>SlotCard — Composite (Depth 2, owns CSS)</h2>
    <p class="text-meta">
      A card is a fixed, NAMED layout template whose slots are TYPED — SUI
      renders each slot, the client supplies the value. The factory is
      SUI-internal; these curried templates are the entire public surface.
    </p>
    <TextBody>
      Each card reacts to its OWN width (container queries). Slots carry a
      priority — low-priority slots drop as the card narrows; the primary slot
      ellipsises rather than dropping.
    </TextBody>
    <PageStack>
      <MaxWidthDemo />
