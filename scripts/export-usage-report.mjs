...
const PKG = "solid-ui-components";
// Consumers live under the PrimeStage umbrella, a couple levels above this repo.
...
function addExport(name, isType) {
...
collectFromFile(join(srcDir, "index.ts"));

const allExports = new Set([...valueExports, ...typeExports]);

// ---------------------------------------------------------------------------
// 2. DISCOVER consumers dynamically.
// ---------------------------------------------------------------------------

function dependsOnSui(pkgJsonPath) {
  try {
...
// Scan one and two levels under the umbrella for package.json files.
const consumerRoots = new Set();
function scanForConsumers(dir, depth) {
  if (depth < 0) return;
  let entries;
  try {
  ...
  if (existsSync(pkgPath) && dependsOnSui(pkgPath)) consumerRoots.add(dir);
  for (const e of entries) {
    if (!e.isDirectory()) continue;
    if (e.name === "node_modules" || e.name.startsWith(".")) continue;
    scanForConsumers(join(dir, e.name), depth - 1);
  }
}
scanForConsumers(umbrella, 3);

// Never treat the SUI repo itself as a consumer.
consumerRoots.delete(repoRoot);

// ---------------------------------------------------------------------------
// 3. COLLECT imported identifiers per consumer.
// ---------------------------------------------------------------------------
...
// Per consumer: { name, root, names:Set, namespaceFiles:[], detail:Map<name,Set<file>> }
const consumers = [];
// Global: name -> Set of consumer names
const usage = new Map();
// Broken: name -> [{consumer, file}]
...
function parseClause(clause, file, consumerName, rec) {
  ...
  if (ns) {
    namespaceImports.push({ consumer: consumerName, file, alias: ns[1] });
  }
...
for (const root of [...consumerRoots].sort()) {
  const name = relative(umbrella, root) || root;
  const files = [];
