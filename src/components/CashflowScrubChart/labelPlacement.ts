...
const boxesTouch = (a: Box, b: Box, gap: number): boolean =>
  a.x0 < b.x1 + gap &&
  b.x0 < a.x1 + gap &&
  a.y0 < b.y1 + gap &&
  b.y0 < a.y1 + gap;
...
const boxInsidePlot = (box: Box, plot: PlotRect): boolean =>
  box.x0 >= plot.left &&
  box.x1 <= plot.right &&
  box.y0 >= plot.top &&
  box.y1 <= plot.bottom;
 ...
 */
const segmentHitsBox = (a: LabelPoint, b: LabelPoint, box: Box): boolean => {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
...
const crossesAnySeries = (box: Box, polylines: readonly Polyline[]): boolean =>
  some(
    (line: Polyline) =>
      some(
