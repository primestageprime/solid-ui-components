...
// ── Pairing for paint ───────────────────────────────────────────────────────
//
// `placeLabels` answers WHERE. A caller that wants to DRAW the result still
// needs the label's own TEXT back, and needs the dropped labels gone. Both
// adapters (`Chart`'s Labels slot and `ScrubChart`'s `ScrubChartLabels`) do
// this same pairing before they paint, so it lives here rather than being
// written twice — this is still geometry-and-text, not JSX, so it stays in
// the core module per the ADR.
