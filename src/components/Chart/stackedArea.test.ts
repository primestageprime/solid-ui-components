...
describe("buildStackedArea", () => {
	...
	it("is NaN-free, in every path it emits", () => {
		...
		expect(every((path: string) => !path.includes("Infinity"), paths)).toBe(
			true,
		);
	});
...
// ── stackBuckets — the column mark's pure half ──────────────────────────────
//
// `stackSegments` cuts on CHANGES, so months holding the same figure collapse
// into one wide stretch. A column chart needs the caller's grid instead, and
// these pin the difference.
describe("stackBuckets", () => {
	const SERIES: readonly StackedSeries[] = [
		{ id: "flat", points: [{ at: 0, value: 10 }] },
		{
			id: "spike",
			points: [
				{ at: 0, value: 0 },
	...
	it("returns one bucket per edge pair, not one per change", () => {
		expect(stackBuckets(SERIES, EDGES)).toHaveLength(4);
		// The same series through the change-cut path collapses to fewer.
		expect(stackSegments(SERIES, [0, 4]).length).toBeLessThan(4);
	});

	it("holds each series' value at the bucket's START", () => {
		const spike = map(
			(b: StackSegment) => b.values[1],
			stackBuckets(SERIES, EDGES),
		);
		// Bucket 2 is the only one that pays, and it pays the WHOLE figure —
		// never a share of it spread onto its neighbours.
		expect(spike).toEqual([0, 0, 90, 0]);
	});

	it("stacks bottom-first, so the last level is the bucket total", () => {
		const buckets = stackBuckets(SERIES, EDGES);
		for (const bucket of buckets) {
			expect(bucket.levels[0]).toBe(0);
			expect(bucket.levels[bucket.levels.length - 1]).toBe(sum(bucket.values));
		}
		expect(buckets[2].levels[buckets[2].levels.length - 1]).toBe(100);
	});

	it("sorts the edges, so a caller cannot make a backwards bucket", () => {
		const jumbled = stackBuckets(SERIES, [3, 0, 4, 2, 1]);
		expect(every((b: StackSegment) => b.to > b.from, jumbled)).toBe(true);
		expect(map((b: StackSegment) => b.from, jumbled)).toEqual([0, 1, 2, 3]);
	});

	it("is empty without at least two edges, and without series", () => {
		expect(stackBuckets(SERIES, [0])).toEqual([]);
		expect(stackBuckets([], EDGES)).toEqual([]);
	});
});
