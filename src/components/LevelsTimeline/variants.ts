...
// component keeps that name at the package root. Live consumers
// (thorcasting-ui, the scenario board) are written against `LevelsTimeline` +
// `LevelsTimelineProps`, and a curried surface under the same name would
// reject the `formatValue` they pass. A base component and a curried variant
// side by side is the ordinary SUI shape (Button, Fab); one name meaning two
// things is not.
//
// ONE variant, deliberately (SUI: start with one, expand only when a real
// caller demands it). A second would have to bake a FORMAT, and a format is
// domain knowledge — dollars, hours, kilograms — which is exactly what this
// component refuses to hold. A consumer that wants one builds it where that
