...
describe("computeEnterFrame — full-component enter: top grows, panes sum to total", () => {
  ...
  it("REVERSE (unresolve): top SHRINKS, bottom grows, panes still sum to total", () => {
    // Unresolve removes a row from done (top shrinks oldTop>newTop) and adds it to
    ...
    for (const p of [0, 0.25, 0.5, 0.75, 1]) {
      const f = rev(p);
      expect(f.topHeight + f.bottomHeight).toBe(480); // sum-to-total every frame
    }
    // The bottom GREW by exactly the one row the top lost.
    expect(rev(1).bottomHeight - rev(0).bottomHeight).toBe(40);
  });
});
...
describe("computeSplitLayout — degenerate inputs", () => {
  ...
  it("an explicit topFloorRows:1 still pins a 1-row floor (back-compat)", () => {
    const r = computeSplitLayout({
      ...base,
      resolvedCount: 0,
      unresolvedCount: 5,
    ...
    expect(r.topHeight).toBe(paneFor(1)); // 60 — opt back into the old floor
  });
});
