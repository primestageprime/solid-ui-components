...
export function computeBoundaryBadges(
  ...
  for (const s of summaries) {
    if (s.column < centerColVal) {
      ...
      const edge = edges.find((e) => e.sourceId === s.id || e.targetId === s.id);
  ...
  return sides.flatMap((g) => {
    ...
    return [
      { key: g.key, d: "", badgeX, badgeY: sideY, pillTopY, pillBottomY, count: g.count },
    ];
  });
}
...
export function computeViewBounds(input: ViewBoundsInput): ViewBounds {
  const { positions, boundaryBadges, bottomBadges, badgeRadius, edgeGutter } = input;
  if (positions.size === 0) {
